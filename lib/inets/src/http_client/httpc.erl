...
% common types
-export_type([request_id/0, profile/0, socket_opt/0]).

% HTTP request types
-export_type([ method/0, http_request/0
             , header/0, content_type/0, status_code/0
             , body/0, http_body_result/0, request_result/0]).

% SSL options
-export_type([ ssloption/0, http_option/0]).

% HTTP Option types
-export_type([ option_request/0, stream_to/0, body_format/0
             , receiver/0, receiver_module/0, receiver_function/0, receiver_args/0
             , cookie_header_opt/0]).
...
-type http_request() :: {uri_string:uri_string(), [header()]} | {uri_string:uri_string(), [header()], content_type(), body()}.
-type status_line() :: {http_version(), status_code(), string()}.
-type http_version() :: uri_string:uri_string().
-type status_code() :: non_neg_integer().
-type content_type() :: uri_string:uri_string().
-type header() :: {field(), value()}.
-type field() :: [byte()].
-type value() :: binary() | iolist().
-type body() :: iolist()
              | binary()
              | { fun((accumulator()) -> eof | {ok, iolist(), accumulator()}), accumulator()}
              | { chunkify, fun((accumulator()) -> eof | {ok, iolist(), accumulator()}), accumulator() }.
-type accumulator() :: term().
-type http_body_result() :: uri_string:uri_string() | binary().

%% HTTP results type `request_result()'.
-type request_result() :: {status_line(), [header()], http_body_result()}
                        | {status_code(), http_body_result()}
                        | request_id()
                        | saved_to_file.
...
-type stream_to() :: none | self | {self, once} | file:name_all().
...
-spec request(uri_string:uri_string()) -> {ok, request_result()} | {error, term()}.
request(Url) ->
    request(Url, default_profile()).

-spec request(Url, Profile) -> {ok, request_result()} | {error, term()} when
...
-spec request(Method, Request, HttpOptions, Options) -> {ok, request_result()} | {error, term()} when
      Method :: method(),
...
-spec request(Method, Request, HttpOptions, Options, Profile) -> {ok, request_result()} | {error, term()} when
      Method :: method(),
      Request :: http_request(),
      HttpOptions :: [http_option()],
      Options :: [option_request()],
...
-spec request_options_sanity_check([option_request()]) -> ok | no_return().
