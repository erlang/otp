...
init_per_testcase(remote_socket_close_high_load = Case, Config0) ->
    ...
    %% on the system
    MaxConnectionsOpen = trunc((FreeMem div MEMORY_FOR_ONE_CONNECTION) * 0.2),
    case MaxConnectionsOpen =:= 0 orelse
        ...
        false ->
            Config = [{profile, Profile}, {max_connections_open, 5} | Config0],
            GivenOptions = proplists:get_value(httpc_options, Config, []),
            ok = httpc:set_options([{max_connections_open, 5} | GivenOptions], Profile),
...
remote_socket_close_high_load(Config0) when is_list(Config0) ->
    ...
    ClientNumber = MaxConnectionsOpen * 2,
    ...
    case request(Config) of
        {ok, {{_,200,"OK"}, _, Body}} ->
            case process_info(self(), message_queue_len) of
                {message_queue_len,0} ->
                    case length(Body) =:= byte_size(?DATA_20MB) of
                        true ->
