...
- Enhance simple_one_for_one error handling. Avoid crash of transient child returning ignore for init-function. Also disallow automatic shutdown as it does not make sense for this supervisor type, may potentially be backwards incompatible for no intended uses, that is supervisors will not remain bug-compatible.
