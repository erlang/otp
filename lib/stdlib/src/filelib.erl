...
wildcard(Pattern, Cwd, Mod) when is_list(Pattern), is_list(Cwd), is_atom(Mod) ->
...
-spec find_file(filename(), filename()) -> {ok, filename()} | {error, not_found}.
...
-type find_source_rule() ::
    {ObjExtension :: string(), SrcExtension :: string(), [find_file_rule()]}.
