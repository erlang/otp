...
system_time_to_local_time(Time, TimeUnit) ->
    SystemTimeSecs = erlang:convert_time_unit(Time, TimeUnit, second),
    Secs = SystemTimeSecs + ?SECONDS_FROM_0_TO_1970,
    gregorian_seconds_to_datetime(Secs + local_offset()).
...
system_time_to_universal_time(Time, TimeUnit) ->
    LocalDateTime = system_time_to_local_time(Time, TimeUnit),
    erlang:localtime_to_universaltime(LocalDateTime).
...
local_offset() ->
    UT = erlang:universaltime(),
    LT = erlang:localtime(),
    case erlang:universaltime() of
        UT ->
            UTime = calendar:datetime_to_gregorian_seconds(UT),
            LTime = calendar:datetime_to_gregorian_seconds(LT),
            LTime - UTime;
        _ -> % Ensure UT and LT within the same second.
            local_offset()
    end.
