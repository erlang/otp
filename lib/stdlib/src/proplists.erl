...
values. For each entry in `ListIn`, if it is associated with some key `K1` such
...
terms). For each pair `{Property, Expansion}` in `Expansions`: if `E` is the
...
-doc """
Partitions `List` into a list of sublists and a remainder. `Lists` contains one
sublist for each key in `Keys`, in the corresponding order. The relative order
of the elements in each sublist is preserved from the original `List`. `Rest`
contains the elements in `List` that are not associated with any of the
