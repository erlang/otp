...
add_element(E, S0) ->
    ...
    case add_bkt_el(E, Bkt, Bkt) of
        not_changed ->
            S0;
        NewBkt ->
            S1 = update_bucket(S0, Slot, NewBkt),
            maybe_expand(S1)
    end.

-spec add_bkt_el(T, [T], [T]) -> {[T] | not_changed}.
add_bkt_el(E, [E|_], _) -> not_changed;
add_bkt_el(E, [_|B], Bkt) ->
    add_bkt_el(E, B, Bkt);
add_bkt_el(E, [], Bkt) -> [E|Bkt].
...
-spec del_bkt_el(T, [T]) -> {[T] | not_changed}.
del_bkt_el(E, [E|Bkt]) -> Bkt;
del_bkt_el(E, [Other|Bkt0]) ->
    Bkt1 = del_bkt_el(E, Bkt0),
    [Other|Bkt1];
del_bkt_el(_, []) -> not_changed.
