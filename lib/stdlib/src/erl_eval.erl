...
eval_generate([V|Rest], P, Anno, Bs0, Lf, Ef, CompFun, AllowSkipping, Acc) ->
    case match(P, V, Anno, new_bindings(Bs0), Bs0, Ef) of
	{match,Bsn} ->
	    Bs2 = add_bindings(Bsn, Bs0),
	    NewAcc = CompFun(Bs2, Acc),
	    eval_generate(Rest, P, Anno, Bs0, Lf, Ef, CompFun, AllowSkipping, NewAcc);
	nomatch when AllowSkipping ->
	    eval_generate(Rest, P, Anno, Bs0, Lf, Ef, CompFun, AllowSkipping, Acc);
    nomatch ->
        apply_error({badmatch, V}, ?STACKTRACE, Anno, Bs0, Ef, none)
	end;
eval_generate([], _P, _Anno, _Bs0, _Lf, _Ef, _CompFun, _AllowSkipping, Acc) ->
    Acc;
eval_generate(Term, _P, Anno, Bs0, _Lf, Ef, _CompFun, _AllowSkipping, _Acc) ->
    apply_error({bad_generator,Term}, ?STACKTRACE, Anno, Bs0, Ef, none).

eval_b_generate(<<_/bitstring>>=Bin, P, Anno, Bs0, Lf, Ef, CompFun, AllowSkipping, Acc) ->
	...
	{match, Rest, Bs1} ->
	    ...
	    NewAcc = CompFun(Bs2, Acc),
	    eval_b_generate(Rest, P, Anno, Bs0, Lf, Ef, CompFun, AllowSkipping, NewAcc);
	{nomatch, Rest} when AllowSkipping ->
	    eval_b_generate(Rest, P, Anno, Bs0, Lf, Ef, CompFun, AllowSkipping, Acc);
    {nomatch, _Rest} ->
...
eval_b_generate(Term, _P, Anno, Bs0, _Lf, Ef, _CompFun, _AllowSkipping, _Acc) ->
    apply_error({bad_generator,Term}, ?STACKTRACE, Anno, Bs0, Ef, none).

eval_m_generate(Iter0, P, Anno, Bs0, Lf, Ef, CompFun, AllowSkipping, Acc0) ->
        ...
        {K,V,Iter} ->
            case match(P, {K,V}, Anno, new_bindings(Bs0), Bs0, Ef) of
                {match,Bsn} ->
                    ...
                    eval_m_generate(Iter, P, Anno, Bs0, Lf, Ef, CompFun, AllowSkipping, Acc);
                nomatch when AllowSkipping ->
                    eval_m_generate(Iter, P, Anno, Bs0, Lf, Ef, CompFun, AllowSkipping, Acc0);
                nomatch ->
...
is_generator({generate_ns,_,_,_}) -> true;
is_generator({b_generate,_,_,_}) -> true;
is_generator({b_generate_ns,_,_,_}) -> true;
is_generator({m_generate,_,_,_}) -> true;
is_generator({m_generate_ns,_,_,_}) -> true;
