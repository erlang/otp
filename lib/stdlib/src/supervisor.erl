...
-type child_spec() ::
    ...
    {Id :: child_id(), StartFunc :: mfargs(), Restart :: restart(), Shutdown :: shutdown(),
...
handle_call({terminate_child, Id}, _From, State) when not is_pid(Id), ?is_simple(State) ->
...
handle_call(which_children, _From, State) when ?is_simple(State) ->
    ...
    Reply = dyn_map(
        fun
            (?restarting(_)) -> {undefined, restarting, CT, Mods};
...
handle_call(which_children, _From, State) ->
    ...
    {reply, Resp, State};
handle_call(count_children, _From, #state{dynamic_restarts = Restarts} = State) when
    ?is_simple(State)
->
    #child{child_type = CT} = get_dynamic_child(State),
    Sz = dyn_size(State),
    % Restarts is always 0 for temporary children
    Active = Sz - Restarts,
    Reply =
        case CT of
            supervisor ->
...
terminate_children(Children, SupName) ->
    Terminate = fun
        (_Id, Child) when ?is_temporary(Child) ->
...
wait_dynamic_children(
    #child{shutdown = brutal_kill} = Child,
    Pids,
    Sz,
    TRef,
    EStack
...
find_child_by_pid(Pid, #state{children = {_Ids, Db}}) ->
    Fun = fun
        (_Id, #child{pid = P} = Ch, _) when P =:= Pid ->
            throw(Ch);
        (_, _, error) ->
            error
    end,
