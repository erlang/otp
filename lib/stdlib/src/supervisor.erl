...
sup_flags() = #{strategy => strategy(),           % optional
                ...
                dynamic_subtree => boolean()}         % optional, available since OTP 28.4
```
...
may specify the ´dynamic_subtree' optional flag to enable the supervisor
...
-type sup_flags() :: #{strategy => strategy(),           % optional
		       ...
		       hibernate_after => timeout(),     % optional
                       dynamic_subtree => boolean()}         % optional
...
init_children(State, StartSpec) ->
    ...
    DynSubTree = (State#state.args =:= [dyn_subtree]),
        ...
        fun(Id,Child) ->
                case do_start_child(SupName, Child, info_report) of
                    {ok, undefined} when ?is_temporary(Child) ->
                        remove;
                    {ok, Pid} when DynSubTree, ?is_temporary(Child) ->
                        do_not_save_start_args(Pid, Child);
                    {ok, Pid} ->
                        {update,Child#child{pid = Pid}};
                    {ok, Pid, _Extra} when DynSubTree, ?is_temporary(Child) ->
                        do_not_save_start_args(Pid, Child);
                    {ok, Pid, _Extra} ->
                        {update,Child#child{pid = Pid}};
                    {error, Reason} ->
...
maybe_save_sup_init_args(_, true) ->
    ...
    %% optional dynamic_subtree flag. Save special value
...
validDynSubTree(What) ->
    throw({invalid_dynamic_subtree, What}).
