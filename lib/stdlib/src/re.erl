...
> on nodes other than the one that compiled it. This was unsafe as it only could
> work if the two nodes were compatible enough (hardware and software) and any
> unpleasant effects could be the result of executing incompatible `mp()` terms.
>
...
node. If not compatible, the `import/1` function will re-compile the regular
expression from its string format which is included in
[`exported()`](`t:exported/0`) as a fallback.
""".
...
- [](){: #EXPORT_OPT }**`export`** - Returns the compiled regular expression on
