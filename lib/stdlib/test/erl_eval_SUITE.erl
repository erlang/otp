...
all() ->
    [guard_1, guard_2, match_pattern, string_plusplus,
     ...
     eep49, binary_and_map_aliases, eep58, non_skipping_generators].
...
non_skipping_generators(Config) when is_list(Config) ->
    ...
    error_check("#{X => X+1 || X := ok <-:- #{1 => ok, 2 => error, 3 => ok}}.",
                {badmatch,{2,error}}),
    %% Binary generators don't skip unused bits at the end either
    error_check("[X || <<X:3>> <=:= <<0>>].",
                {badmatch,<<0:2>>}),
