...
state_enter(_Config) ->
    ...
    [{3,{enter,start}},
     ...
     {6,{state_timeout,3}},
     {7,{state_timeout,4}},
     {8,{state_timeout,5}}] = flush(),
    {wait,[9|_]} = sys:get_state(STM),
    {9,repeat,wait} = gen_statem:call(STM, repeat),
    [{9,enter,wait,wait},
     {10,{timeout,9}},
     {11,{state_timeout,9}}] = flush(),
    {12,echo,wait} = gen_statem:call(STM, echo),
    [{13,enter,start,wait},
     {14,internal,start,wait}] = flush(),
    {15,repeat,start} = gen_statem:call(STM, repeat),
    [{16,enter,start,start}] = flush(),
    {17,stop} = gen_statem:call(STM, {stop,bye}),
    [{'EXIT',STM,bye}] = flush(),
