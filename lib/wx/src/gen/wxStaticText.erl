...
wxWidgets docs:
[wxStaticText](https://docs.wxwidgets.org/3.1/classwx_static_text.html)
""".
-include("wxe.hrl").
...
new(#wx_ref{type=ParentT}=Parent,Id,Label, Options)
 when is_integer(Id),?is_chardata(Label),is_list(Options) ->
  ...
  wxe_util:queue_cmd(Parent,Id,Label_UC, Opts,?get_env(),?wxStaticText_new_4),
...
create(#wx_ref{type=ThisT}=This,#wx_ref{type=ParentT}=Parent,Id,Label, Options)
 when is_integer(Id),?is_chardata(Label),is_list(Options) ->
  ...
  MOpts = fun({pos, {_posX,_posY}} = Arg) -> Arg;
          ...
          (BadOpt) -> erlang:error({badoption, BadOpt}) end,
  Opts = lists:map(MOpts, Options),
  wxe_util:queue_cmd(This,Parent,Id,Label_UC, Opts,?get_env(),?wxStaticText_Create),
