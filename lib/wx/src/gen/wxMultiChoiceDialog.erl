...
wxWidgets docs:
[wxMultiChoiceDialog](https://docs.wxwidgets.org/3.1/classwx_multi_choice_dialog.html)
""".
-include("wxe.hrl").
...
new(#wx_ref{type=ParentT}=Parent,Message,Caption,Choices, Options)
 when ?is_chardata(Message),?is_chardata(Caption),is_list(Choices),is_list(Options) ->
  ...
  wxe_util:queue_cmd(Parent,Message_UC,Caption_UC,Choices_UCA, Opts,?get_env(),?wxMultiChoiceDialog_new),
