...
wxWidgets docs:
[wxPageSetupDialogData](https://docs.wxwidgets.org/3.1/classwx_page_setup_dialog_data.html)
""".
-include("wxe.hrl").
...
the currently selected printer properties (Windows only).
""".
...
as orientation) instead of showing a dialog (Windows only).
""".
...
-doc """
Returns the right (x) and bottom (y) minimum margins the user can enter (Windows
only).
...
`m:wxPrintData` object, otherwise the paper size overrides the paper id.
""".
