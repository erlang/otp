...
-export([cacheBestSize/2,canSetTransparent/1,captureMouse/1,center/1,center/2,
  ...
  scrollWindow/4,sendSizeEvent/1,sendSizeEvent/2,setAcceleratorTable/2,
  ...
  show/2,showFullScreen/2,showFullScreen/3,thaw/1,transferDataFromWindow/1,
...
-spec new(Bitmap, SplashStyle, Milliseconds, Parent, Id, [Option]) -> wxSplashScreen() when
	...
	Option :: {'pos', {X::integer(), Y::integer()}}
		 ...
		 | {'style', integer()}.
new(#wx_ref{type=BitmapT}=Bitmap,SplashStyle,Milliseconds,#wx_ref{type=ParentT}=Parent,Id, Options)
 when is_integer(SplashStyle),is_integer(Milliseconds),is_integer(Id),is_list(Options) ->
  ...
  wxe_util:queue_cmd(Bitmap,SplashStyle,Milliseconds,Parent,Id, Opts,?get_env(),?wxSplashScreen_new),
