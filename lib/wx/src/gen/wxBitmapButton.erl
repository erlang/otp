...
wxWidgets docs:
[wxBitmapButton](https://docs.wxwidgets.org/3.1/classwx_bitmap_button.html)
...
-export([cacheBestSize/2,canSetTransparent/1,captureMouse/1,center/1,center/2,
  ...
  getBestSize/1,getBitmapDisabled/1,getBitmapFocus/1,getBitmapLabel/1,
  getCaret/1,getCharHeight/1,getCharWidth/1,getChildren/1,getClientSize/1,
  getContainingSizer/1,getContentScaleFactor/1,getCursor/1,getDPI/1,
  getDPIScaleFactor/1,getDropTarget/1,getExtraStyle/1,getFont/1,getForegroundColour/1,
  ...
  setContainingSizer/2,setCursor/2,setDefault/1,setDoubleBuffered/2,
...
new(#wx_ref{type=ParentT}=Parent,Id,#wx_ref{type=BitmapT}=Bitmap, Options)
 when is_integer(Id),is_list(Options) ->
  ...
  wxe_util:queue_cmd(Parent,Id,Bitmap, Opts,?get_env(),?wxBitmapButton_new_4),
...
-spec create(This, Parent, Id, Bitmap, [Option]) -> boolean() when
	...
	Option :: {'pos', {X::integer(), Y::integer()}}
		 ...
		 | {'style', integer()}
		 | {'validator', wx:wx_object()}.
create(#wx_ref{type=ThisT}=This,#wx_ref{type=ParentT}=Parent,Id,#wx_ref{type=BitmapT}=Bitmap, Options)
 when is_integer(Id),is_list(Options) ->
  ...
  MOpts = fun({pos, {_posX,_posY}} = Arg) -> Arg;
          ...
          (BadOpt) -> erlang:error({badoption, BadOpt}) end,
  Opts = lists:map(MOpts, Options),
  wxe_util:queue_cmd(This,Parent,Id,Bitmap, Opts,?get_env(),?wxBitmapButton_Create),
...
newCloseButton(#wx_ref{type=ParentT}=Parent,Winid)
 when is_integer(Winid) ->
  ?CLASS(ParentT,wxWindow),
  wxe_util:queue_cmd(Parent,Winid,?get_env(),?wxBitmapButton_NewCloseButton),
...
%% @hidden
-doc false.
setBitmapLabel(This,Bitmap) -> wxButton:setBitmapLabel(This,Bitmap).
%% @hidden
-doc false.
setBitmapFocus(This,Bitmap) -> wxButton:setBitmapFocus(This,Bitmap).
%% @hidden
-doc false.
setBitmapDisabled(This,Bitmap) -> wxButton:setBitmapDisabled(This,Bitmap).
%% @hidden
-doc false.
getBitmapLabel(This) -> wxButton:getBitmapLabel(This).
%% @hidden
-doc false.
getBitmapFocus(This) -> wxButton:getBitmapFocus(This).
%% @hidden
-doc false.
getBitmapDisabled(This) -> wxButton:getBitmapDisabled(This).
%% @hidden
-doc false.
