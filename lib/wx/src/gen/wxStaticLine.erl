...
wxWidgets docs:
[wxStaticLine](https://docs.wxwidgets.org/3.1/classwx_static_line.html)
""".
-include("wxe.hrl").
...
new(#wx_ref{type=ParentT}=Parent, Options)
 when is_list(Options) ->
  ...
  wxe_util:queue_cmd(Parent, Opts,?get_env(),?wxStaticLine_new_2),
...
create(#wx_ref{type=ThisT}=This,#wx_ref{type=ParentT}=Parent, Options)
 when is_list(Options) ->
  ...
  MOpts = fun({id, _id} = Arg) -> Arg;
          ...
          (BadOpt) -> erlang:error({badoption, BadOpt}) end,
  Opts = lists:map(MOpts, Options),
  wxe_util:queue_cmd(This,Parent, Opts,?get_env(),?wxStaticLine_Create),
