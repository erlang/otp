  ...
  FoldFun =
	...
	case NotCalled of
	  true ->
		...
		error -> {false, {}};
		{ok, {_M, F, A} = MFA} ->
		  {not is_nowarn_unused_function(MFA, AccState1),
		   {unused_fun, [F, A]}}
	      end,
	    case Warn of
	      true -> state__add_warning(AccState1, ?WARN_NOT_CALLED, Fun, Msg);
	      false -> AccState1
	    end;
	  false ->
	    {Name, Contract} =
...
is_nowarn_unused_function(MFA, #state{codeserver = CServer}) ->
  dialyzer_utils:is_nowarn_unused_fun(MFA, CServer).
