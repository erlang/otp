    ...
    case dialyzer_cl_parse:start() of
        ...
        {gui, Opts} ->
            try
                ...
                throw:{dialyzer_error, Msg} -> cl_error(Msg)
            end,
...
get_plt_info([PLT | PLTs]) ->
    String =
        case dialyzer_plt:included_files(PLT) of
            {ok, Files} ->
                io_lib:format(
                    "The PLT ~ts includes the following files:\n~tp\n\n",
                    [PLT, Files]
                );
            {error, read_error} ->
    ...
    case OutputFile =:= none of
        ...
        false ->
            case file:open(OutputFile, [write]) of
                ...
                {error, Reason} ->
                    Msg1 = io_lib:format(
                        "Could not open output file ~tp, Reason: ~p\n",
                        [OutputFile, Reason]
                    ),
...
format_warning({_Tag, {File, Line}, Msg}, Opts) when is_list(File), is_integer(Line) ->
    F =
        case proplists:get_value(filename_opt, Opts, basename) of
            fullpath -> File;
...
message_to_string({bin_construction, [Culprit, Size, Seg, Type]}, I) ->
    io_lib:format(
        "Binary construction will fail since the ~s field ~s in"
...
message_to_string({fun_app_no_fun, [Op, Type, Arity]}, I) ->
    io_lib:format(
        "Fun application will fail since ~ts :: ~ts"
...
message_to_string({neg_guard_fail, [Guard, Args]}, I) ->
    io_lib:format(
        "Guard test not(~s~ts) can never succeed\n",
        [Guard, a(Args, I)]
    );
message_to_string({guard_fail_pat, [Pat, Type]}, I) ->
    io_lib:format(
        "Clause guard cannot succeed. The ~ts was matched"
        " against the type ~ts\n",
        [ps(Pat, I), t(Type, I)]
    );
...
message_to_string(
    ...
    io_lib:format(
        "The contract ~ts cannot be right because the inferred"
...
message_to_string({invalid_contract, [M, F, A, Sig]}, I) ->
    io_lib:format(
        "Invalid type specification for function ~w:~tw/~w."
...
message_to_string({extra_range, [M, F, A, ExtraRanges, SigRange]}, I) ->
    io_lib:format(
        "The specification for ~w:~tw/~w states that the function"
...
message_to_string({missing_range, [M, F, A, ExtraRanges, ContrRange]}, I) ->
    io_lib:format(
        "The success typing for ~w:~tw/~w implies that the function"
...
message_to_string({overlapping_contract, [M, F, A]}, _I) ->
    io_lib:format(
        "Overloaded contract for ~w:~tw/~w has overlapping domains;"
        " such contracts are currently unsupported and are simply ignored\n",
        [M, F, A]
    );
message_to_string({spec_missing_fun, [M, F, A]}, _I) ->
    io_lib:format(
        "Contract for function that does not exist: ~w:~tw/~w\n",
        [M, F, A]
    );
...
message_to_string({opaque_eq, [Type, _Op, OpaqueType]}, I) ->
    io_lib:format(
        "Attempt to test for equality between a term of type ~ts"
...
message_to_string({callback_type_mismatch, [B, F, A, ST, CT]}, I) ->
    io_lib:format(
        "The inferred return type of ~tw/~w ~ts has nothing in"
...
message_to_string({callback_arg_type_mismatch, [B, F, A, N, ST, CT]}, I) ->
    io_lib:format(
        "The inferred type for the ~s argument of ~tw/~w (~ts) is"
...
message_to_string({callback_spec_type_mismatch, [B, F, A, ST, CT]}, I) ->
    io_lib:format(
        "The return type ~ts in the specification of ~tw/~w is not a"
...
message_to_string(
    ...
    io_lib:format(
        "The specified type for the ~ts argument of ~tw/~w (~ts) is"
        " not a supertype of ~ts, which is expected type for this"
        " argument in the callback of the ~w behaviour\n",
        [ordinal(N), F, A, t(ST, I), t(CT, I), B]
    );
    ...
    case FailReason of
        only_sig ->
            case ArgNs =:= [] of
                true ->
                    ...
                    io_lib:format(
                        "will never return since the success typing arguments"
                        " are ~ts\n",
                        [t(SigArgs, I)]
                    );
                false ->
                    io_lib:format(
                        "will never return since it differs in the ~s argument"
        ...
        only_contract ->
            case (ArgNs =:= []) orelse IsOverloaded of
                ...
                false ->
                    io_lib:format(
                        "breaks the contract ~ts in the ~s argument\n",
                        [sig(Contract, I), PositionString]
                    )
            end;
        both ->
            io_lib:format(
                "will never return since the success typing is ~ts -> ~ts"
...
form_expected_without_opaque([{N, T, TStr}], I) ->
    case erl_types:t_is_opaque(T) of
        ...
        false ->
            io_lib:format(
                "a term of type ~ts (with opaque subterms) as ",
                [t(TStr, I)]
            )
    ...
    case ExpectedArgs of
        [T] ->
            ...
            case erl_types:t_is_opaque(T) of
                true ->
                    io_lib:format(
                        "an opaque term of type ~ts is expected",
                        [t(TS, I)]
                    );
                false ->
                    io_lib:format(
                        "a structured term of type ~ts is expected",
                        [t(TS, I)]
                    )
            end;
        [_, _ | _] ->
