...
eval_generate([V|Rest], P, Bs0, CompFun, AllowSkipping, Ieval) ->
    case catch match1(P, V, erl_eval:new_bindings(), Bs0) of
	{match,Bsn} ->
	    Bs2 = add_bindings(Bsn, Bs0),
            CompFun(Bs2) ++ eval_generate(Rest, P, Bs0, CompFun, AllowSkipping, Ieval);
	nomatch when AllowSkipping ->
	    eval_generate(Rest, P, Bs0, CompFun, AllowSkipping, Ieval);
        nomatch ->
            exception(error, {badmatch, V}, Bs0, Ieval)
	end;
eval_generate([], _P, _Bs0, _CompFun, _AllowSkipping, _Ieval) ->
    [];
eval_generate(Term, _P, Bs, _CompFun, _AllowSkipping, Ieval) ->
    exception(error, {bad_generator,Term}, Bs, Ieval).

eval_b_generate(<<_/bitstring>>=Bin, P, Bs0, CompFun, AllowSkipping, Ieval) ->
    Mfun = match_fun(Bs0),
	...
	{match,Rest,Bs1} ->
	    ...
	    CompFun(Bs2) ++ eval_b_generate(Rest, P, Bs0, CompFun, AllowSkipping, Ieval);
        {nomatch,Rest} when AllowSkipping ->
            eval_b_generate(Rest, P, Bs0, CompFun, AllowSkipping, Ieval);
        {nomatch,_Rest} ->
            exception(error, {badmatch, Bin}, Bs0, Ieval);
	done when not AllowSkipping, Bin =/= <<>> ->
            exception(error, {badmatch, Bin}, Bs0, Ieval);
        done ->
            []
    end;
eval_b_generate(Term, _P, Bs, _CompFun, _AllowSkipping, Ieval) ->
    exception(error, {bad_generator,Term}, Bs, Ieval).

eval_m_generate(Iter0, P, Bs0, CompFun, AllowSkipping, Ieval) ->
    case maps:next(Iter0) of
        {K,V,Iter} ->
            case catch match1(P, {K,V}, erl_eval:new_bindings(), Bs0) of
                {match,Bsn} ->
                    Bs2 = add_bindings(Bsn, Bs0),
                    CompFun(Bs2) ++ eval_m_generate(Iter, P, Bs0, CompFun, AllowSkipping, Ieval);
                nomatch when AllowSkipping ->
                    eval_m_generate(Iter, P, Bs0, CompFun, AllowSkipping, Ieval);
                nomatch ->
