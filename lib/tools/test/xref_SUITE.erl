...
-export([add/1, default/1, info/1, info_nodebug/1, info/2, lib/1, lib_nodebug/1,
         ...
         unsafe/1, unsafe_nodebug/1, documented/1, documented_nodebug/1]).
    ...
    [{xref, [],
     ...
     {files, [],
      [add, default, info, info_nodebug, lib, lib_nodebug, read, read_nodebug,
       ...
       fun_mfa, fun_mfa_nodebug, fun_mfa_vars, fun_mfa_vars_nodebug,
       qlc, qlc_nodebug, unsafe, documented]},
     {analyses, [],
...
unsafe_nodebug(Conf) when is_list(Conf) ->
    do_unsafe(Conf, false).

do_unsafe(Conf, FromAbst) ->
    put(from_abst, FromAbst),
...
documented(Conf) when is_list(Conf) ->
    do_documented(Conf, true).

documented_nodebug(Conf) when is_list(Conf) ->
    do_documented(Conf, false).

do_documented(Conf, FromAbst) ->
    put(from_abst, FromAbst),
