...
valid_policy_mapping_13(Config) when is_list(Config)->
    run([{"4.10.13.1", "Valid Policy Mapping Test13 EE", ok},
         {"4.10.13.2", "Valid Policy Mapping Test13 EE", ok}
        ]).
...
run({Chap, Test, Result, CertsBody}, TA) ->
	...
	{Result, {_, PolicySet}} ->
            validate_policy_set(Chap, PolicySet);
...
intermidiate_cas(Chap) when Chap == "4.10.13.1";
                            Chap == "4.10.13.2";
                            Chap == "4.10.14" ->
    [
...
crl_names(Chap) when Chap == "4.10.13.1";
                     Chap == "4.10.13.2";
                     Chap == "4.10.14" ->
    ["Trust Anchor Root CRL",
...
validate_policy_set(Chap, Set) when Chap == "4.8.18.2";
    ...
    validate_qualifiers(Chap, Set);
validate_policy_set(Chap, Set) when Chap == "4.10.13.1" ->
    validate_nodes([[{expected_policy_set,[?NIST2_OID]},
                     {valid_policy, ?NIST1_OID}]], Set),
    validate_qualifiers(Chap, Set);
validate_policy_set(Chap, Set) when Chap == "4.10.13.2" ->
    validate_nodes([[{expected_policy_set,[?NIST2_OID]},
                     {valid_policy, ?NIST1_OID}]], Set),
    validate_qualifiers(Chap, Set);
...
validate_nodes([Expected | Rest], Nodes) ->
    do_validate_nodes(Expected, Nodes),
    validate_nodes(Rest, Nodes).


do_validate_nodes(_, []) ->
    true;
do_validate_nodes(Expected, [Node | Nodes]) ->
    try validate_node(Expected, Node) of
        true ->
            true
    catch _:_ ->
            do_validate_nodes(Expected, Nodes)
...
validate_node([], _) ->
     true;
validate_node([{Key, Value}| Expected], Node) ->
     case maps:get(Key, Node) of
         Value ->
             validate_node(Expected, Node);
         _ ->
             throw(false)
     end.
...
validate_qualifiers("4.10.13.1", [#{qualifier_set := QSet1}, #{qualifier_set := QSet2}]) ->
    ct:log("QS1 ~p QS2 ~p", [QSet1, QSet2]),
    true = [#'UserNotice'{explicitText =
...
validate_qualifiers("4.10.13.2", [#{qualifier_set := QSet}]) ->
    ct:log("QS ~p", [QSet]),
    true = [#'UserNotice'{explicitText =
                              {visibleString, "q9:  This is the user notice from qualifier 9 "
                               "associated with NIST-test-policy-1.  This user notice should be"
