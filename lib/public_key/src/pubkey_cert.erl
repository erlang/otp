            ...
            UserState =
                verify_fun(OtpCert, {bad_cert,
                                     {policy_requirement_not_met,
                                      {{explicit_policy, ExplicitPolicyConstraint},
                                       {policy_set, pubkey_policy_tree:constraint_policy_node_set(Tree)}}}},
...
prepare_for_next_cert(OtpCert, #path_validation_state{
    ...
    ValidationState1 =
        ValidationState0#path_validation_state{
          ...
          %% 6.1.4 h
          explicit_policy = maybe_decrement(ExplicitPolicyConstraint, IsSelfSigned),  % 1
          inhibit_policy_mapping = maybe_decrement(PolicyMappingConstraint, IsSelfSigned), % 2
          inhibit_any_policy = maybe_decrement(AnyPolicyConstraint, IsSelfSigned) % 3
         },
...
handle_last_cert(OtpCert, #path_validation_state{last_cert = true,
    ...
    ValidTree = policy_tree_intersection(PolicySet, Tree, CurrentDepth),
    validate_policy_tree(OtpCert,
...
%%   (ii) If there was no match in step (i) and the valid_policy_tree
...
%%   (a) inhibit_anyPolicy is greater than 0 or (b) i<n and the
    ...
    LeafFun =
        fun(#{expected_policy_set := ExpPolicySet,
              valid_policy := ValidPolicy}) ->
                PolicyChildren = policy_children(ExpPolicySet, PolicyInfoList),
                case AnyPolicyConstraint > 0 orelse
                    ((CertNum < PathLen) andalso SelfSigned) of
                    true ->
                        PolicyChildren ++ any_policy_children(AnyExt, ValidPolicy,
                                                              ExpPolicySet, PolicyChildren);
                    false ->
                         PolicyChildren
                end
        end,
    pubkey_policy_tree:add_leaves(Tree, LeafFun).
...
policy_children(ExpPolicySet, PolicyInfoList) ->
    lists:foldl(fun(#'PolicyInformation'{
                       ...
                       policyQualifiers = Qualifiers
                        ...
                        case lists:member(Policy, ExpPolicySet)
                            orelse ExpPolicySet == [?anyPolicy] of
                            true ->
...
any_policy_children(undefined, _,_, _) ->
    [];
any_policy_children(#'PolicyInformation'{
                       policyIdentifier = ?anyPolicy,
                       policyQualifiers = Qualifiers}, ValidPolicy,
                    ExpPolicySet, Children) ->
    [pubkey_policy_tree:policy_node(Policy, Qualifiers, [ValidPolicy])
     || Policy <- ExpPolicySet, not pubkey_policy_tree:in_set(Policy, Children)].
...
handle_policy_mapping_ext(#'PolicyMappings_SEQOF'{
    ...
    case pubkey_policy_tree:map_leaves(Tree, MapPolicy) of
        Tree ->
            pubkey_policy_tree:add_siblings(Tree, AnySiblings);
        NewTree ->
            NewTree
    end.
...
policy_tree_intersection([?anyPolicy], Tree, _) -> % (ii) from RFC
    Tree;
policy_tree_intersection(UserPolicySet, Tree0, CurrentDepth) ->
    case pubkey_policy_tree:is_empty(Tree0) of
        true ->  % (i) from RFC
            Tree0;
        false -> % (iii) from RFC
            ...
            Tree = handle_any_policy_leaves(Tree1, ValidPolicyNodeSet, UserPolicySet, CurrentDepth),
            pubkey_policy_tree:prune_tree(Tree)
    end.
...
handle_any_policy_leaves(Tree, _, [?anyPolicy], _) ->
    Tree;
handle_any_policy_leaves(Tree0, ValidPolicyNodeSet, UserPolicySet, CurrentDepth) ->
    case pubkey_policy_tree:any_leaf(Tree0, CurrentDepth) of
        no_node ->
            Tree0;
        #{qualifier_set := Qualifiers}->
            ...
            Tree = pubkey_policy_tree:add_siblings(Tree0, Siblings),
