...
defaults to `false`. The legacy value `true` will enable you retrive keyloging from
the connection in a polling manner and does not work for failing connections.
...
`{keylog_hs, fun()}` to retrive keylogs on all key update events or to
retrive keylog if the connection fails during the handshake. Note that enabling this is for
debug purposes and defeats the purpose of the TLS protocol, so use with care.
