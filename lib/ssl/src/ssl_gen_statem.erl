...
handle_own_alert(Alert0, StateName,
		 #state{static_env = #static_env{role = Role,
                                                 protocol_cb = Connection} = StaticEnv,
                        ssl_options = #{log_level := LogLevel} = Opts} = State) ->
    Alert = Alert0#alert{role = Role},
    maybe_store_alert_src(maps:get(keep_secrets, Opts, undefined), StateName, StaticEnv),
...
maybe_store_alert_src({keylog_hs, Fun}, 
                      StateName, #static_env{transport_cb = Transport,
                                             socket = Socket,
                                             protocol_cb = Connection}) when StateName =/= connection
                                                                             andalso is_function(Fun) ->
    case Connection of
        tls_gen_connection ->
            case tls_socket:sockname(Transport, Socket) of
                {ok, SockName} ->
                    put(src, SockName);
                _  ->
                    ok
        ...
        dtls_gen_connection ->
            case dtls_socket:sockname(Transport, Socket) of
                {ok, SockName} ->
                    put(src, SockName);
                _  ->
                    ok
...
maybe_store_alert_src(_,_,_) ->
    ok.

maybe_keylog_hs_callback(#alert{level = ?FATAL}, StateName,
                            #state{static_env = #static_env{} = SEnv,
                                   ssl_options = #{keep_secrets := {keylog_hs, Fun}}} 
                         = State) when is_function(Fun)  ->
    case keylog_hs_alert(StateName, State) of
        [] ->
            ok;
        KeyLog ->
            Fun(hs_keylog_info(KeyLog, SEnv))
    end;
maybe_keylog_hs_callback(_, _, _) ->
    ok.

hs_keylog_info(KeyLog, #static_env{role = client, host = Host, port = Port}) ->
    Src = alert_src(),
    #{items => KeyLog, server => {Host, Port}, src => Src};
hs_keylog_info(KeyLog, #static_env{role = server, port = Port}) ->
    Src = alert_src(),
    #{items => KeyLog, listen_port => Port, src => Src}.
...
keylog_hs_alert(_, #state{connection_env =
                          ...
                          connection_states = ConnectionStates})
  when ?TLS_GTE(TlsVersion, ?TLS_1_3) ->
    CSRead = ssl_record:pending_connection_state(ConnectionStates, read),
    keylog_hs_1_3(CSRead);
