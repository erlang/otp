...
- **`{keep_secrets, KeepSecrets}`** - Configures a TLS connection for keylogging.
  ...
  > and enabling it makes the user responisble for the information
  ...
  If set to legacy value `true` keylog information can be retrived from the connection
  ...
  > to be retrieved in a polling manner.
...
opt_keep_secrets(UserOpts, Opts) ->
    case get_opt(keep_secrets, false, UserOpts, Opts) of
        ...
        {new, {FunType, Fun} = Value} when FunType == keylog orelse FunType == keylog_hs, is_function(Fun) ->
            Opts#{keep_secrets => Value};
        {old, _} ->
            Opts;
