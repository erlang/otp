...
- **`{keep_secrets, KeepSecrets}`** - Configures a TLS connection for keylogging.
  ...
  The `keep_secrets` can also be set to a fun of arity one, in which case
  secrets will be kept and the fun will be called with a map with the keylog
  items and some socket information, keylog_info(), in the case that the connection fails. The
