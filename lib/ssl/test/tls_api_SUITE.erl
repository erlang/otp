...
get_protected_clientfile_opts(FileNameString,Config) ->
    ClientOpts = ssl_test_lib:ssl_options(client_rsa_opts, Config),
    {ok,ClientKeyFile} = file:read_file(proplists:get_value(keyfile, ClientOpts)),
    [PemEntry] = public_key:pem_decode(ClientKeyFile),
    Params = {"DES-CBC",
              {'PBES2-params',
    ...
    ProtectedPemEntry = public_key:pem_entry_encode(
                          ...
                          {Params,?CORRECT_PASSWORD}),
    ProtectedClientKeyFile = filename:join(
