...
-export([socket_stream_recv/2, socket_stream_send/2,
         socket_stream_controlling_process/2]).
...
socket_stream(Socket) ->
    {socket_stream_in(Socket), socket_stream_out(Socket),
     fun ?MODULE:socket_stream_controlling_process/2}.

socket_stream_in(Socket) ->
    [fun ?MODULE:socket_stream_recv/2 | Socket].

socket_stream_recv(InStream = [_ | Socket], Size) ->
    case
        if
            Size =:= 0 ->
    ...
    of
        {ok, Data} ->
            [Data | InStream];
        {error, {Reason, _Data}} ->
            socket_stream_recv_error(InStream, Reason);
        {error, timeout} ->
            [<<>> | InStream];
        {error, Reason} ->
            socket_stream_recv_error(InStream, Reason)
...
socket_stream_recv_error(InStream, closed) ->
    [closed | InStream];
socket_stream_recv_error(_, Reason) ->
    erlang:error({?MODULE, ?FUNCTION_NAME, Reason}).

socket_stream_out(Socket) ->
    [fun ?MODULE:socket_stream_send/2 | Socket].

socket_stream_send(OutStream, Bin) when is_binary(Bin) ->
    socket_stream_send(OutStream, [Bin]);
socket_stream_send(OutStream = [_ | Socket], Data) ->
...
socket_stream_controlling_process(Stream = {_, [_ | Socket], _}, Pid) ->
    %%
