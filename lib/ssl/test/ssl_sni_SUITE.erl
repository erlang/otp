...
recv_and_certificate(SSLSocket) ->
    ...
    rdnPart(Subject, ?'id-at-commonName').
...
run_handshake(Config, SNIHostname, ExpectedSNIHostname, ExpectedCN) ->
    ...
    ssl_test_lib:check_result(Server, ExpectedSNIHostname, Client, {utf8String, binary:list_to_bin(ExpectedCN)}).
