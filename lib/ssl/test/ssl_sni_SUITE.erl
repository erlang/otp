...
dns_name(Config) ->
    ...
    unsuccessfull_connect(ServerConf, [{verify, verify_peer} | ClientConf], undefined, Config, handshake_failure),
...
no_ip_fallback(Config) ->
    ...
    ClientConf = ssl_test_lib:sig_algs(rsa, Version) ++ ClientOpts0,
    successfull_connect(ServerConf, [{verify, verify_peer} | ClientConf], Hostname, Config),
    unsuccessfull_connect(ServerConf, [{verify, verify_peer} | ClientConf], IP, Config, handshake_failure),
    unsuccessfull_connect(ServerConf, [{verify, verify_peer} | ClientConf], IPStr, Config, handshake_failure).
...
unsuccessfull_connect(ServerOptions, ClientOptions, Hostname0, Config, Alert) ->
    {ClientNode, ServerNode, Hostname1} = ssl_test_lib:run_where(Config),
    Hostname = host_name(Hostname0, Hostname1),
    Server = ssl_test_lib:start_server_error([{node, ServerNode}, {port, 0},
					      {from, self()},
