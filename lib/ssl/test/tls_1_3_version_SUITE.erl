...
keylog_on_alert() ->
    [{doc,"Test that keep_secrets callback, if specified, "
      "is called with keylog info if alert is raised"}].
...
keylog_on_alert(Config) when is_list(Config) ->
    ...
    Me = self(),
    Fun = fun(Keylog) ->
                  Me ! {alert_keylog, Keylog}
          end,
    alert_passive([{keep_secrets, Fun} | ServerOpts], ClientOpts, recv,
                  ServerNode, Hostname),
    receive
        {alert_keylog, SKeyLog} ->
            case SKeyLog of
    ...
    alert_passive(ServerOpts, [{keep_secrets, Fun} | ClientOpts], recv,
                  ServerNode, Hostname),
    receive
        {alert_keylog, CKeyLog} ->
            case CKeyLog of
