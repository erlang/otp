...
seq_trace(Config) when is_list(Config) ->
   ...
   %% Additional test for metatrace
	...
	Pid when is_pid(Pid) ->
	    ...
	    test_server:fail("metatrace failed for startproc")
    end,
    case P1Proc of
	{P1,_,_} -> ok;
	P1 when is_pid(P1) ->
	    ...
	    io:format("Trace information file:\n~p\n",
			 ...
			 filename:join(Privdir,
				       atom_to_list(Node)++"-seq_trace.ti"))]),
	    test_server:fail("metatrace failed for P1")
    end,
    case P2Proc of
	{P2,_,_} -> ok;
	P2 when is_pid(P2) ->
	    ...
	    io:format("Trace information file:\n~p\n",
			 ...
			 filename:join(Privdir,
				       atom_to_list(Node)++"-seq_trace.ti"))]),
	    test_server:fail("metatrace failed for P2")
    end,
    ok.
...
%	    test_server:fail("metatrace failed on "++atom_to_list(Node))
...
check_gone(Dir,File) ->
    ...
    ?line case lists:member(File,List) of
		  ...
		  case lists:member(File,NewList) of
			  ...
			  io:format("~p: ~p~n",
				    [Dir,NewList]),
			  test_server:fail(File ++ " not removed from original place");
		      false ->
