...
get_portinfo(Fd,Port) ->
	...
	"Monitors" ->
	    Monitors0 = string:lexemes(val(Fd),"()"),
	    Monitors = [begin
...
get_etsinfo(Fd,EtsTable = #ets_table{details=Ds},WS) ->
	...
	"Buckets" ->
	    ...
	    Buckets = list_to_integer(string:trim(val(Fd),both,"\s")),
	...
	"Chain Length Avg" ->
	    Val = try list_to_float(string:trim(val(Fd),both,"\s"))
                  catch _:_ -> "-"
                  end,
