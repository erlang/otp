   ...
   try
      ...
      case State#state.log of
          false -> rpc:block_call(State#state.node, rb, start, []),
                   ...
                   {true, "Enabled. rb_server started on observed node."};
          true  -> rpc:block_call(State#state.node, rb, stop, []),
                   set_status("Observer - " ++ atom_to_list(State#state.node)),
                   {false, "Disabled. rb_server stopped on observed node."}
      end
      of
          {Bool, Msg} -> UpdState = State#state{log=Bool},
                         create_txt_dialog(Frame, Msg, "Log view status", ?wxICON_EXCLAMATION),
                         {noreply, UpdState}
      catch
          _:Reason -> create_txt_dialog(Frame, Reason, "Log view status", ?wxICON_ERROR),
                      {noreply, State}
   end;
...
is_sasl_started(Node) ->
   ...
   case lists:keyfind(sasl, 1, Apps) of
       false       ->  throw("Incompatible. sasl application not started."),
                       error ;
       {sasl, _, _}  ->  ok
   end.
...
is_mf_h_handler_used(Node) ->
   ...
   case lists:any(fun(L)-> L == log_mf_h end, Handlers) of
       false -> throw("Incompatible. log_mf_h handler not used in sasl."),
                error;
       true  -> ok
   end.
...
ensure_rb_module_loaded(Node) ->
   ...
   case rpc:block_call(Node, code, ensure_loaded, [rb]) of
       {badrpc, Reason} -> throw("badrpc : " ++ io_lib:format("~tp",[Reason])),
                           error ;
       {error, Reason}  -> throw("rb module load error : " ++ io_lib:format("~tp",[Reason])),
                           error ;
       {module,rb}      -> ok
   end.
...
is_rb_compatible(Node) ->
   ...
   case rpc:block_call(Node, erlang, function_exported, [rb, log_list, 0]) of
       false -> throw("Incompatible. Node's Erlang release must be at least R16B2.");
       true  -> ok
   end.
...
is_rb_server_running(Node, LogState) ->
   ...
   case rpc:block_call(Node, erlang, whereis, [rb_server]) of
       Pid when is_pid(Pid),
                (LogState == false) -> throw("Locked. rb_server is already started and maybe used by someone."),
                                       already_started ;
