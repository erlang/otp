...
vann(Tree, Env) ->
    case erl_syntax:type(Tree) of
        ...
        non_skipping_generator ->
            vann_non_skipping_generator(Tree, Env);
        binary_generator ->
            vann_binary_generator(Tree, Env);
        non_skipping_binary_generator ->
            vann_non_skipping_binary_generator(Tree, Env);
        map_generator ->
            vann_map_generator(Tree, Env);
        non_skipping_map_generator ->
            vann_non_skipping_map_generator(Tree, Env);
        block_expr ->
...
vann_list_comp_body_join() ->
    fun (T, {Env, Bound, Free}) ->
            {T1, Bound1, Free1} = case erl_syntax:type(T) of
                                      ...
                                      non_skipping_generator ->
                                          vann_non_skipping_generator(T, Env);
                                      binary_generator ->
                                          vann_binary_generator(T,Env);
                                      non_skipping_binary_generator ->
                                          vann_non_skipping_binary_generator(T,Env);
                                      map_generator ->
                                          vann_map_generator(T,Env);
                                      non_skipping_map_generator ->
                                          vann_non_skipping_map_generator(T,Env);
                                      _ ->
...
vann_non_skipping_generator(Tree, Env) ->
    P = erl_syntax:non_skipping_generator_pattern(Tree),
    {P1, Bound, _} = vann_pattern(P, []),
    E = erl_syntax:non_skipping_generator_body(Tree),
    {E1, _, Free} = vann(E, Env),
    Tree1 = rewrite(Tree, erl_syntax:non_skipping_generator(P1, E1)),
    {ann_bindings(Tree1, Env, Bound, Free), Bound, Free}.
...
vann_non_skipping_binary_generator(Tree, Env) ->
    P = erl_syntax:non_skipping_binary_generator_pattern(Tree),
    {P1, Bound, _} = vann_pattern(P, Env),
    E = erl_syntax:non_skipping_binary_generator_body(Tree),
    {E1, _, Free} = vann(E, Env),
    Tree1 = rewrite(Tree, erl_syntax:non_skipping_binary_generator(P1, E1)),
    {ann_bindings(Tree1, Env, Bound, Free), Bound, Free}.
...
vann_non_skipping_map_generator(Tree, Env) ->
    P = erl_syntax:non_skipping_map_generator_pattern(Tree),
    {P1, Bound, _} = vann_pattern(P, []),
    E = erl_syntax:non_skipping_map_generator_body(Tree),
    {E1, _, Free} = vann(E, Env),
    Tree1 = rewrite(Tree, erl_syntax:non_skipping_map_generator(P1, E1)),
    {ann_bindings(Tree1, Env, Bound, Free), Bound, Free}.
