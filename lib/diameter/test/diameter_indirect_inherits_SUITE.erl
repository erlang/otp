...
-define(GROUPED_DICT_A,
    ...
    get_grouped_definitions_from_avps(?DEFAULT_AVPS_WITH_GROUPS) ++
...
-define(VENDOR_DICT_B(VendorIds),
    ...
    lists:join("\n", VendorIds) ++
    get_grouped_definitions_from_avps(?DEFAULT_AVPS ++ ?VENDOR_AVPS_A ++ ?VENDOR_AVPS_B) ++
    get_messages_from_avps("B", "2", ?DEFAULT_AVPS ++ ?VENDOR_AVPS_A ++ ?VENDOR_AVPS_B))
).
...
-define(VENDOR_DICT_C(VendorIds),
    ...
    lists:join("\n", VendorIds) ++
    get_grouped_definitions_from_avps(?DEFAULT_AVPS ++ ?VENDOR_AVPS_A ++ ?VENDOR_AVPS_C) ++
    get_messages_from_avps("C", "3", ?DEFAULT_AVPS ++ ?VENDOR_AVPS_A ++ ?VENDOR_AVPS_B ++ ?VENDOR_AVPS_C))
).
...
verify_vendor_ids_are_indirectly_inherited(_) ->
    ...
    ct:pal("~s~n~s~n", [HA, EA]),
    diameter_test_a = MA = load_forms(FA),
    verify_avps(MA, ?DEFAULT_AVPS ++ ?VENDOR_AVPS_A, ?VENDOR_AVPS_B ++ ?VENDOR_AVPS_C),

    {ok, [HB, EB, FB]} = codec_list_of_options(DictB),
    ct:pal("~s~n~s~n", [HB, EB]),
    diameter_test_b = MB = load_forms(FB),
    verify_avps(MB, ?DEFAULT_AVPS ++ ?VENDOR_AVPS_A ++ ?VENDOR_AVPS_B, ?VENDOR_AVPS_C),
    verify_group_avps(MB, ?VENDOR_AVPS_B, ?DEFAULT_AVPS ++ ?VENDOR_AVPS_A),

    {ok, [HC, EC, FC]} = diameter_make:codec(DictC, ?OPTS_INHERITS),
    ct:pal("~s~n~s~n", [HC, EC]),
    diameter_test_c = MC = load_forms(FC),
    verify_avps(MC, ?DEFAULT_AVPS ++ ?VENDOR_AVPS_A ++ ?VENDOR_AVPS_B ++ ?VENDOR_AVPS_C),
    verify_group_avps(MC, ?VENDOR_AVPS_B ++ ?VENDOR_AVPS_C, ?DEFAULT_AVPS ++ ?VENDOR_AVPS_A).
...
verify_vendor_ids_can_only_be_overridden_by_inheriting_module(_) ->
    ...
    verify_avps(MB, VB, ?DEFAULT_AVPS ++ ?VENDOR_AVPS_A ++ ?VENDOR_AVPS_B, ?VENDOR_AVPS_C),
    verify_group_avps(MB, ?VENDOR_AVPS_B, ?DEFAULT_AVPS ++ ?VENDOR_AVPS_A),
    ...
    verify_avps(MC, VC, ?DEFAULT_AVPS ++ ?VENDOR_AVPS_A ++ ?VENDOR_AVPS_B ++ ?VENDOR_AVPS_C),
    verify_group_avps(MC, ?VENDOR_AVPS_B ++ ?VENDOR_AVPS_C, ?DEFAULT_AVPS ++ ?VENDOR_AVPS_A).
