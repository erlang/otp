...
gen_rtl(bs_get_position, [Dst], [Ms],
  ...
  case hipe_rtl_arch:word_size() of
    4 -> %% 32-bit
      ...
      Pos = hipe_rtl:mk_new_reg_gcsafe(),
      {[Offset, SaveOffset], ExtractOffsets} = extract_matchstate_vars([offset,{saveoffset,0}], Ms),
      [ExtractOffsets,
       hipe_rtl:mk_alu(Pos, Offset, sub, SaveOffset),
...
make_matchstate(Binary, Max, Ms, TrueLblName, FalseLblName, IsNewMatch) ->
  ...
  [hipe_rtl:mk_gctest(?MS_MIN_SIZE+Max+1),
...
resize_matchstate(Ms, Max, TrueLblName) ->
  ...
  [hipe_rtl:mk_gctest(?MS_MIN_SIZE+Max+1),
   ...
   hipe_tagscheme:create_matchstate(Max, BinSize, Base, Offset, Orig, Ms, false),
   hipe_rtl:mk_goto(TrueLblName)].
