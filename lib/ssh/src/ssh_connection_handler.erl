...
handle_event(info, {Proto, Sock, NewData}, StateName,
    ...
    {error, exceeds_max_decompressed_size} ->
            Details = "Bad packet: Size after decompression exceeds max size",
            {Shutdown, D} =
                ?SEND_DISCONNECT(?SSH_DISCONNECT_PROTOCOL_ERROR, Details, StateName, D1),
            {stop, Shutdown, D}
    catch
...
terminate(Reason, StateName, D0) ->
    ...
    log(error, D0, Reason),
    DisconnectReason = "Internal error",
    Details = io_lib:format("Reason: ~p",[Reason]),
    {_ShutdownReason, D} =
        ?SEND_DISCONNECT(?SSH_DISCONNECT_BY_APPLICATION, DisconnectReason, Details, StateName, D0),
    close_transport(D).
...
call_disconnectfun_log_cond(DisconnectContext, D) ->
...
%%%----------------------------------------------------------------
disconnect_fun(#{details := Details}, D) ->
    disconnect_fun(Details, D);
disconnect_fun(Details, D) ->
    ?CALL_FUN(disconnectfun,D)(Details).
...
ssh_dbg_on(disconnect) -> dbg:tpl(?MODULE,  send_disconnect, 2, x),
                          dbg:tpl(?MODULE,  send_disconnect, 3, x).
...
ssh_dbg_off(disconnect) -> dbg:ctpl(?MODULE, send_disconnect, 2),
                           dbg:ctpl(?MODULE, send_disconnect, 3);
...
ssh_dbg_format(disconnect, {call,{?MODULE,send_disconnect,
                                  [#{code := Code, details := Details,
                                     ...
                                     state_name := StateName}, _D]}}) ->
    ["Disconnecting:\n",
     io_lib:format(" Module = ~p, Line = ~p, StateName = ~p,~n"
                   " Code = ~p,~n"
                   " Details =~n"
                   " ~p",
                   [Module, Line, StateName, Code, lists:flatten(Details)])
    ];
ssh_dbg_format(disconnect, {call,{?MODULE,send_disconnect,
...
ssh_dbg_format(renegotiation, {return_from, {?MODULE,send_disconnect,2}, _Ret}) ->
    skip;
ssh_dbg_format(renegotiation, {return_from, {?MODULE,send_disconnect,3}, _Ret}) ->
    skip.
