 ...
 * factory extending the OtpNamedTransportFactory abstract class.
 *
...
public class OtpNode extends OtpLocalNode {
    ...
    public OtpNode(final String node,
        ...
        if (transportFactory instanceof OtpNamedTransportFactory) {
            ...
            // node name as the identifier for incoming connections.
            init(this.node);
        } else {
            init(0);
        }
    ...
    /*
     * Intialize a node instance with the port number to use for incoming
    ...
    /*
     * Intialize a node instance using an alternative distribution protocol
     * with the node name passed as the identifier to use for incoming
     * connections.
     */
    private synchronized void init(final String node) throws IOException {
        if (!initDone) {
            connections = new Hashtable<String, OtpCookedConnection>(17,
                    (float) 0.95);
            mboxes = new Mailboxes();
            acceptor = new Acceptor(node);
            initDone = true;
        }
    }
    ...
    public class Acceptor extends Thread {
         ...
         */
        Acceptor(final String node) throws IOException {
            // Set acceptorPort arbitrarily to 0, it won't be used later on.
            acceptorPort = 0;

            // The node name is passed as the identifier to use for incoming
            // connections.
        ...
        private void unPublishPort() {
            ...
            if (OtpNode.this.transportFactory instanceof
                OtpNamedTransportFactory) {
                return;
            }
