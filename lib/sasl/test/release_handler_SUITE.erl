...
instructions(Conf) when is_list(Conf) ->
    ...
    case whereis(cc) of
        ...
        _ -> test_server:fail("cc not started")
    end,
    ...
    if
        SecondBB =:= FirstBB -> test_server:fail("bb not killed");
        true -> ok
    end,
    ...
    case ThirdBB of
        ...
        undefined -> test_server:fail("bb not started")
    end,
...
check_bstate(Slogan,ExpectedProcs) ->
	...
	Slogan =:= "no", BB =/= undefined ->
	    test_server:fail("instructions failed; process bb is running");
	Slogan =/= "no", BB =:= undefined ->
	    test_server:fail("instructions failed; process bb is not running");
	ExpectedProcs2 =:= [], ActualProcs =/= ExpectedProcs2 ->
	    test_server:fail("instructions failed; old b processes are running");
	ActualProcs =/= ExpectedProcs2 ->
	    test_server:fail("instructions failed; wrong number of old b processes are running");
	true ->
	    ok
    end.
...
no_cc() ->
    case whereis(cc) of
        Pid when is_pid(Pid) -> test_server:fail("cc not stopped");
        _ -> ok
    end.
...
start_target_node_with_erl(Erl,Sname,Boot) ->
	...
	ok ->
	    ...
	    {ok,FullName};
	Error ->
            test_server:fail({failed_to_start_node, FullName, Error})
    end.
...
target_system_unicode(Conf) when is_list(Conf) ->
	...
	Error -> test_server:fail({failed_to_start_sasl_on_test_node,Node,Error})
    end,
...
are_names_reg_gg(Node, Names, N) ->
	...
	Regs ->
	    test_server:fail({error, {"Names not registered",
			     {{"should :", Names},
			      {"was :", Regs}}}})
    end.
...
copy_error(Src, Dest, Reason) ->
    ...
    test_server:fail(file_copy_failed).
...
check_gg_info(Node,OtherAlive,OtherDead,Synced,N) ->
    ...
    try do_check_gg_info(OtherAlive,OtherDead,Synced,GGI,GI) 
	    ...
	    test_server:fail("check_gg_info failed");
	  _:E:Stacktrace ->
...
start_node_unix(Sname,NodeDir) ->
	...
	ok ->
	    {ok,node_name(Sname)};
        Error ->
            test_server:fail({failed_to_start_node, Sname, Error})
    end.
...
wait_installed(Node,RelVsn,0) ->
    test_server:fail("install_release_changed_gg failed for " ++ RelVsn ++
