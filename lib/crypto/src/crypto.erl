...
- **`argon2id`, `argon2i`, `argon2d`** - The Argon2 password hashing
  ...
  RFC 9106 recommends `memory => 65536, iterations => 3, parallelism => 4`
  as a first choice, and `memory => 65536, iterations => 1, parallelism => 4`
  if a lower running time is required.
...
```erlang
1> Key = crypto:kdf(argon2id, <<"correct horse battery staple">>, 32,
