...
static int check_pkey_algorithm_type(ErlNifEnv *env, const int alg_arg_num, ERL_NIF_TERM algorithm, const int allow_unknown,
                                     ERL_NIF_TERM *err_return)
{
    if (
#ifndef HAVE_EDDSA
                                ...
                                ERL_NIF_TERM *err_return)
{
	...
	tbs = md_value;
        ...
        tbslen = static_cast<size_t>(tbsleni);
    }
                                ...
                                ERL_NIF_TERM *err_return)
{
    ...
    if (enif_is_map(env, argv[key_arg_num])) {
        ...
        *pkey = ENGINE_load_private_key(e, id, nullptr, password);
        if (!*pkey)
...
#else
        pkey_format_t pkey_format = PKEY_PRIV;
...
int get_pkey_from_octet_string(ErlNifEnv *env,
                               ...
                               ERL_NIF_TERM key_bin, pkey_format_t pkey_format,
                               const pkey_type_t *pkey_type,
                               EVP_PKEY **pkey_p,
                               ERL_NIF_TERM *ret_p)
{
    ...
    if (pkey_type) {
        ctx = EVP_PKEY_CTX_new_id(pkey_type->evp_pkey_id, nullptr);
        if (!ctx) {
                               ...
                               ERL_NIF_TERM *err_return)
{
    ...
    if (enif_is_map(env, argv[key_arg_num])) {
        ...
        password = get_key_password(env, argv[key_arg_num]);
        *pkey = ENGINE_load_public_key(e, id, nullptr, password);
        if (!*pkey)
...
ERL_NIF_TERM pkey_sign_heavy_nif(ErlNifEnv *env, const int argc, const ERL_NIF_TERM argv[])
{
...
{/* (Algorithm, Type, Data|{digest,Digest}, Key|#{}, Options) */
    ...
    pkey_type_t *pkey_type = get_pkey_type(argv[0]);
    EVP_PKEY_CTX *ctx = nullptr;
    size_t siglen;
#else
    int len;
...
# endif
    ...
    unsigned char *tbs = nullptr; /* data to be signed */
    size_t tbslen = 0;
...
# ifdef HAVE_EDDSA
        if (!FIPS_MODE()) {
            ...
            if (EVP_DigestSign(mdctx, nullptr, &siglen, tbs, tbslen) != 1)
...
# endif
        ...
        if ((ctx = EVP_PKEY_CTX_new(pkey, nullptr)) == nullptr) {
...
# ifdef HAVE_RSA_PKCS1_PSS_PADDING
            if (sig_opt.rsa_padding == RSA_PKCS1_PSS_PADDING) {
                if (sig_opt.rsa_mgf1_md != nullptr) {
...
# endif
        ...
        if (EVP_PKEY_sign(ctx, nullptr, &siglen, tbs, tbslen) != 1)
                                  ...
                                  ERL_NIF_TERM *err_return)
{
    ...
    if (argv[algorithm_arg_num] == atom_rsa) {
        opt->rsa_mgf1_md = nullptr;
        opt->rsa_oaep_label.data = nullptr;
        opt->rsa_oaep_label.size = 0;
        opt->rsa_oaep_md = nullptr;
        opt->rsa_padding = RSA_PKCS1_PADDING;
        opt->signature_md = nullptr;
    } else {
        opt->rsa_mgf1_md = nullptr;
        opt->rsa_oaep_label.data = nullptr;
        opt->rsa_oaep_label.size = 0;
    ...
    while (enif_get_list_cell(env, tail, &head, &tail)) {
        ...
        if (tpl_terms[0] == atom_rsa_padding
            ...
            if (!get_pkey_digest_type(env, argv[algorithm_arg_num], nullptr,
                                      options_arg_num, tpl_terms[1],
                                      &opt_md, err_return))
                goto err; /* An exception is present in ret */
...
{/* (Algorithm, Data, PublKey=[E,N]|[E,N,D]|[E,N,D,P1,P2,E1,E2,C], Options, IsPrivate, IsEncrypt) */
    ...
    size_t tmplen;
#endif
    uint8_t *label_copy = nullptr;

    const int is_private = argv[4] == atom_true;
    int is_encrypt = argv[5] == atom_true;
    ...
    if (is_private) {
        if (!get_pkey_private_key(env, argv, 0, 2, nullptr, &pkey, &ret))
            goto err; /* An exception is present in ret */
    } else {
        if (!get_pkey_public_key(env, argv, 0, 2, nullptr, &pkey, &ret))
            goto err;  /* An exception is present in ret */
    }
    ...
    if (argv[0] == atom_rsa) {
        if (crypt_opt.signature_md != nullptr) {
...
# ifdef HAVE_RSA_OAEP_MD
        if (crypt_opt.rsa_padding == RSA_PKCS1_OAEP_PADDING) {
            if (crypt_opt.rsa_oaep_md != nullptr) {
            ...
            if (crypt_opt.rsa_mgf1_md != nullptr) {
            ...
            if (crypt_opt.rsa_oaep_label.data != nullptr && crypt_opt.rsa_oaep_label.size > 0) {
                if (crypt_opt.rsa_oaep_label.size > INT_MAX)
                    ASSIGN_GOTO(ret, err, EXCP_BADARG_N(env, 3, "RSA oep label too large"));
                if ((label_copy = reinterpret_cast<uint8_t *>(OPENSSL_malloc(crypt_opt.rsa_oaep_label.size))) ==
                    nullptr)
                    goto err;
...
# endif
    ...
    if (is_private)
        result =
            is_encrypt ? EVP_PKEY_sign(ctx, nullptr, &outlen, in_bin.data, in_bin.size)
                       : EVP_PKEY_decrypt(ctx, nullptr, &outlen, in_bin.data, in_bin.size);
    else
        result =
            is_encrypt ? EVP_PKEY_encrypt(ctx, nullptr, &outlen, in_bin.data, in_bin.size)
                       : EVP_PKEY_verify_recover(ctx, nullptr, &outlen, in_bin.data, in_bin.size);
...
{ /* (Algorithm, PrivKey | KeyMap) */
    ...
    if (!get_pkey_private_key(env, argv, 0, 1, nullptr, &pkey, &ret)) // handles engine
        goto err; /* An exception is present in ret */
