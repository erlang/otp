...
ERL_NIF_TERM kem_availability_t::get_atom() const { return this->init->atom; }
...
//
bool kem_availability_t::check_kem_algorithm(bool fips_enabled) {
#ifdef HAVE_ML_KEM
#if defined(FIPS_SUPPORT) && defined(HAS_3_0_API)
    const auto_evp_kem_t kem(EVP_KEM_fetch(nullptr, this->init->str_v3, nullptr));
    if (!kem) {
        return false; // not available by name
    }

    const auto_evp_pkey_ctx_t ctx(EVP_PKEY_CTX_new_from_name(nullptr, this->init->str_v3, nullptr));
...
void kem_probe_t::probe(ErlNifEnv* env, const bool fips_enabled, std::vector<kem_availability_t>& output) {
    ...
    kem_availability_t algo = {.init = this};
#if defined(FIPS_SUPPORT) && defined(HAS_3_0_API)
