...
/* (OtherPublicKey, {CurveDef,CurveName}, My) */
{
    ...
    peer_pctx = EVP_PKEY_CTX_new_from_name(nullptr, "EC", nullptr);

    gcd.use_curve_name = 1;
retry_without_name:
    /* Curve definition/name */
    if (!get_curve_definition(env, &ret, argv[1], params, &i, nullptr, &gcd))
        goto err;
    ...
    */
    if (!get_ossl_BN_param_from_bin(env, const_cast<char*>("priv"),  argv[2], &params[i_key]))
        ASSIGN_GOTO(ret, err, EXCP_BADARG_N(env, 0, "Bad peer public key; binary expected"));

    own_pctx = EVP_PKEY_CTX_new_from_name(nullptr, "EC", nullptr);
    ...
    if (!(pctx_gen = EVP_PKEY_CTX_new(own_pkey, nullptr)))
    ...
    if ((err = EVP_PKEY_derive(pctx_gen, nullptr, &sz)) <= 0)
