...
// Called on fips mode change to reset the algorithm lists. Next lazy_init call to each collection will do the work
// again.
...
public:
    ...
    // Resets the found algorithms list and the flag for lazy init, so lazy init will
    void reset() {
