...
// Array lookup
extern "C" digest_availability_Cptr get_digest_type(ERL_NIF_TERM type) {
    for (auto& p : digest_collection) {
        if (type == p.get_atom()) {
            return digest_availability_Cptr{.ptr = &p};
        }
    }
    return digest_availability_Cptr{}; // sorry, nullptr
}
...
extern "C" bool is_digest_forbidden_in_fips(const digest_availability_Cptr p) {
    if (p.ptr == nullptr) {
