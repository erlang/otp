...
{/* (Type, KeyMaterial, KeyLen, Options) */
    ...
    ErlNifBinary key_material, salt, secret, ad, info, out;
    int out_allocated = 0;
    ERL_NIF_TERM ret;
    uint64_t memory, iterations, parallelism, scn, scr, scp, maxmem;
    ErlNifUInt64 keylen;
    size_t n = 0;
    ...
    params[n++] =
        OSSL_PARAM_construct_octet_string((char*)kdf->input_name,
