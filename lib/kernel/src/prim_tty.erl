...
-nifs([isatty/1, tty_create/0, tty_init/2, setlocale/1,
       tty_select/3, tty_window_size/1,
...
reader([TTY, Encoding, Parent]) ->
    ...
    reader_loop(#{ tty => TTY, parent => Parent, signal => SignalRef,
                   reader => ReaderRef, enc => FromEnc, read => 0,
                   ready_input => false}, <<>>).

reader_loop(State = #{ tty := TTY, parent := Parent, signal := SignalRef,
...
setlocale(_TTY) ->
    erlang:nif_error(undef).
tty_select(_TTY, _SignalRef, _ReaderRef) ->
    erlang:nif_error(undef).
tty_encoding(_TTY) ->
    erlang:nif_error(undef).
