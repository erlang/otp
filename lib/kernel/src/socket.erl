...
-doc """
SO_TIMESTAMPING flag atom.

An atom representing a timestamping flag for the `{socket, timestamping}` socket
option. Multiple flags can be combined in a list to enable multiple timestamping
features simultaneously. The list may also contain integer values for flags that
are not available as atoms (e.g., flags introduced in newer kernel versions).

The available flag atoms (compatible with Linux kernel 3.0+):

- **`timestamping_rx_hardware`** - Request RX timestamps generated by the network adapter
- **`timestamping_rx_software`** - Request RX timestamps when data enters the kernel
- **`timestamping_tx_hardware`** - Request TX timestamps generated by the network adapter
- **`timestamping_tx_software`** - Request TX timestamps when data leaves the kernel
- **`timestamping_software`** - Report any software timestamps when available
- **`timestamping_raw_hardware`** - Report hardware timestamps as generated by TX/RX hardware flags

For other flags (such as `SOF_TIMESTAMPING_TX_SCHED`, `SOF_TIMESTAMPING_TX_ACK`,
`SOF_TIMESTAMPING_TX_COMPLETION`, `SOF_TIMESTAMPING_OPT_ID`),
you can pass the integer flag value directly in the list if your kernel supports them.

See the [Linux kernel documentation](https://docs.kernel.org/networking/timestamping.html)
for detailed information about timestamp generation and reporting.

Example:
```erlang
socket:setopt(Sock, socket, timestamping, [timestamping_rx_software, timestamping_tx_software])
socket:setopt(Sock, socket, timestamping, [timestamping_rx_software, 64])  % 64 = SOF_TIMESTAMPING_TX_SCHED
```
""".
-type timestamping() ::
        timestamping_rx_hardware |
        timestamping_rx_software |
        timestamping_tx_hardware |
        timestamping_tx_software |
        timestamping_software    |
        timestamping_raw_hardware.
...
- **`{socket, timestamping}`** - `Value = [` `t:timestamping/0` `]`

  Enable timestamping features using the `SO_TIMESTAMPING` socket option. The
  value is a list of `t:timestamping/0` atoms representing the timestamping flags
  to enable. Multiple flags can be combined in the list. An empty list `[]`
  disables all timestamping.

  This option is Linux-specific. See `t:timestamping/0` for available flags and
  the [Linux kernel documentation](https://docs.kernel.org/networking/timestamping.html)
