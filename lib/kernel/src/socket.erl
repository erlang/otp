...
-spec send(Socket, Data, Flags, nowait) ->
                  ok |
                  {select, SelectInfo} |
                  {ok, {RestData, SelectInfo}} |
                  {error, Reason} when
...
-spec sendmsg(Socket, Msg, Flags) -> ok | {error, Reason} when
      Socket :: socket(),
      Msg    :: msg_send(),
      Flags  :: [msg_flag()],
      Reason :: posix() | closed;
      ...
      Reason       :: posix() | closed;
             ...
             (Socket, Msg, Timeout) -> ok | {error, Reason} when
      Socket  :: socket(),
      Msg     :: msg_send(),
      Timeout :: timeout(),
...
-spec recvmsg(Socket, Flags, nowait)
             -> {ok, Msg} |
                {select, SelectInfo} |
                {error, Reason} when
      Socket     :: socket(),
