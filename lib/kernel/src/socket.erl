...
- **`rx_hardware`** - Request RX timestamps generated by the network adapter
- **`rx_software`** - Request RX timestamps when data enters the kernel
- **`tx_hardware`** - Request TX timestamps generated by the network adapter
- **`tx_software`** - Request TX timestamps when data leaves the kernel
- **`software`** - Report any software timestamps when available
- **`raw_hardware`** - Report hardware timestamps as generated by TX/RX hardware flags
...
Example:
```erlang
socket:setopt(Sock, socket, timestamping, [rx_software, tx_software])
socket:setopt(Sock, socket, timestamping, [rx_software, 64])  % 64 = SOF_TIMESTAMPING_TX_SCHED
```
""".
-type timestamping() ::
        rx_hardware |
