...
-record(gconf, {parameter_value = invalid   :: [group_tuple()]
                ...
                own_publish_type = invalid  :: publish_type() | invalid,
                group_name = []             :: group_name() | [],
                group_publish_type = normal :: publish_type(),
                group_list = []             :: [node()],
...
init([]) ->
    ...
    S = initial_group_setup(fetch_new_group_conf(), Conns, #{}),
    {ok, S}.
...
initial_group_setup(#gconf{group_name = DefGroupName,
    ...
    #state{node_name = node(), group_publish_type = PubTpGrp,
...
%%%====================================================================================
continue_handle_info({nodeup, Node, _Info},
                     #state{connections = Conns,
                            rpc_requests = Reqs}) when Node == node() ->
    %% Check configuration since we now know how to interpret it
    %% when we know our name...
    S = initial_group_setup(alive_state_change_group_conf(), Conns, Reqs),
...
%%%====================================================================================
continue_handle_info({nodedown, Node, _Info}, #state{node_name = Node} = S) ->
...
new_group_conf(KernParamValue) ->
    ...
    case persistent_term:get(?MODULE, #gconf{}) of
        ...
        #gconf{own_publish_type = OldType} ->
            OwnPublishType = if OldType == invalid -> publish_arg();
                                true -> OldType
                             end,
...
get_own_nodes() ->
    get_own_nodes(lookup_group_conf()).
...
group_configured() ->
    group_configured(lookup_group_conf()).
