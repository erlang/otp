...
%% %CopyrightEnd%
%%
-module(gen_tcp_blocking_dist).

%%
%% This is a modification of gen_tcp_dist that adds possibility to simulate
%% network outage between nodes.
%% This currently behaves in similar way to inet_tcp_proxy: https://github.com/rabbitmq/inet_tcp_proxy
%% When on Node1 we block Node2, we:
%% - stop consuming distribution messages (we allow them to be queued, but not sent, they will be
%%   sent in bulk when node is unblocked later)
%% - stop sending ticks
%% - raise exit exception from setup/5 if node we want to connect to is blocked
%% - raise exit exception after handshake with blocked node completes
%%

%% Public interface for blocking network communication
-export([block_peer/2, unblock_peer/2,
         block_pair/2, unblock_pair/2,
         unblock_pair_ignore_node_down/2, has_network_blocker/0,
         set_peer_ref/2, get_peer_ref/1, get_peer_ref/2]).

%% gen_tcp_dist interface
...
-export([dist_cntrlr_setup/1, dist_cntrlr_input_setup/4,
         dist_cntrlr_tick_handler/2]).

-export([accept_loop/2,do_accept/6,do_setup/6]).

%% Internal exports for rpc calls
-export([block_internal/1, unblock_internal/1]).
...
%% ------------------------------------------------------------
%% Block outgoing communication (and ticks) from node
%% 'From' to node 'To'. This is a one way block.
%% ------------------------------------------------------------
block_peer(From, To) when From =:= node() ->
    ok = block_internal(To);
block_peer(From, To) ->
    ok = remote_call(From, ?MODULE, block_internal, [To]).

%% ------------------------------------------------------------
%% Unblock outgoing communication (and ticks) from node
%% 'From' to node 'To'. This is a one way unblock.
%% ------------------------------------------------------------
unblock_peer(From, To) when From =:= node() ->
    ok = unblock_internal(To);
unblock_peer(From, To) ->
    ok = remote_call(From, ?MODULE, unblock_internal, [To]).

%% ------------------------------------------------------------
%% Block outgoing communication (and ticks) between 2 nodes.
%% This is a two way block.
%% ------------------------------------------------------------
block_pair(Node1, Node2) when Node1 =:= node() ->
    ok = block_peer(Node2, Node1),
    %% Block local outgoing communication last
    ok = block_peer(Node1, Node2);
block_pair(Node1, Node2) ->
    ok = block_peer(Node1, Node2),
    ok = block_peer(Node2, Node1).

%% ------------------------------------------------------------
%% Unblock outgoing communication (and ticks) between 2 nodes.
%% This is a two way unblock.
%% ------------------------------------------------------------
unblock_pair(Node1, Node2) when Node2 =:= node() ->
    %% Unblock local outgoing communication first
    ok = unblock_peer(Node2, Node1),
    ok = unblock_peer(Node1, Node2);
...
%% This is a two way unblock.
%% ------------------------------------------------------------
unblock_pair_ignore_node_down(Node1, Node2) when Node1 =:= node() ->
    %% Unblock local outgoing communication first
    ok = unblock_peer(Node1, Node2),
    try unblock_peer(Node2, Node1) catch error : {badmatch, {badrpc, nodedown}} -> ok end;
unblock_pair_ignore_node_down(Node1, Node2) when Node2 =:= node() ->
    %% Unblock local outgoing communication first
    ok = unblock_peer(Node2, Node1),
    try unblock_peer(Node1, Node2) catch error : {badmatch, {badrpc, nodedown}} -> ok end;
unblock_pair_ignore_node_down(Node1, Node2) ->
    try unblock_peer(Node1, Node2) catch error : {badmatch, {badrpc, nodedown}} -> ok end,
    try unblock_peer(Node2, Node1) catch error : {badmatch, {badrpc, nodedown}} -> ok end.

%% ------------------------------------------------------------
%%  Internal stuff for network blocking
%% ------------------------------------------------------------
block_internal(Node) ->
    persistent_term:put({?MODULE, block, Node}, true).

unblock_internal(Node) ->
    case persistent_term:erase({?MODULE, block, Node}) of
        true ->
            case lists:keysearch(Node, 1, erlang:system_info(dist_ctrl)) of
                {value, {Node, Pid}} ->
                    %% Notify output handler to flush data
                    Pid ! allow,
                    ok;
                _ ->
                    ok
...
is_blocked(Node) ->
    persistent_term:get({?MODULE, block, Node}, false).

remote_call(Node, M, F, A) ->
    case get_peer_ref(Node, undefined) of
        undefined ->
            rpc:call(Node, M, F, A);
        Peer ->
            case is_process_alive(Peer) of
                true ->
                    peer:call(Peer, M, F, A);
                false ->
                    rpc:call(Node, M, F, A)
            end
    end.
...
%% ------------------------------------------------------------
%%  Check if network blocker is enabled on current node
%% ------------------------------------------------------------
has_network_blocker() ->
    Args = init:get_arguments(),
    lists:all(fun(Elem) -> lists:member(Elem, Args) end, ?NETWORK_BLOCKER_DIST_OPTS_PARSED).

set_peer_ref(Node, Ref) ->
    persistent_term:put({?MODULE, peer, Node}, Ref).

get_peer_ref(Node) ->
    persistent_term:get({?MODULE, peer, Node}).

get_peer_ref(Node, Default) ->
    persistent_term:get({?MODULE, peer, Node}, Default).

%% gen_tcp_dist interface starts here:
%% ------------------------------------------------------------
...
do_setup(Kernel, Node, Type, MyNode, LongOrShortNames, SetupTime) ->
    ...
    case is_blocked(Node) of
        true ->
            ?trace("(~p): Node: ~p blocked, aborting setup/5~n", [node(), Node]),
            ?shutdown(Node);
        false ->
...
%% ticks.
%%
dist_cntrlr_tick_handler(Socket, Node0) ->
    receive
        tick ->
            case is_blocked(Node0) of
                true ->
                    %% Don't send tick...
                    ok;
                false ->
        ...
        {set_node, Node} ->
            dist_cntrlr_tick_handler(Socket, Node);
        _ ->
            ok
    end,
    dist_cntrlr_tick_handler(Socket, Node0).
...
dist_cntrlr_setup_loop(Socket, TickHandler, Sup) ->
    receive
        ...
        {Ref, From, {handshake_complete, Node, DHandle}} ->
            case is_blocked(Node) of
                true ->
                    ?trace("(~p): Node: ~p blocked, aborting after handshake_complete~n", [node(), Node]),
                    exit({shutdown, blocked});
                false ->
                    From ! {Ref, ok},
                    TickHandler ! {set_node, Node},
                    ...
                    dist_cntrlr_output_loop(DHandle, Socket, Node)
            end
    end.
...
dist_cntrlr_input_setup(DHandle, Socket, Node, Sup) ->
    link(Sup),
    %% Ensure we don't try to put data before we are registered
    %% as input handler...
    receive
        DHandle ->
            dist_cntrlr_input_loop(DHandle, Socket, Node, 0)
    end.

dist_cntrlr_input_loop(DHandle, Socket, Node, N) when N =< ?ACTIVE_INPUT/2 ->
    inet:setopts(Socket, [{active, ?ACTIVE_INPUT - N}]),
    dist_cntrlr_input_loop(DHandle, Socket, Node, ?ACTIVE_INPUT);
dist_cntrlr_input_loop(DHandle, Socket, Node, N) ->
    receive
        {tcp_closed, Socket} ->
        ...
        {tcp, Socket, Data} ->
            ...
            dist_cntrlr_input_loop(DHandle, Socket, Node, N - 1);
...
dist_cntrlr_output_loop(DHandle, Socket, Node) ->
    receive
        Msg when Msg =:= dist_data; Msg =:= allow ->
            %% Outgoing data from this node...
            case is_blocked(Node) of
                true ->
                    %% Don't flush message from buffer if peer is blocked
                    dist_cntrlr_output_loop(DHandle, Socket, Node);
                false ->
