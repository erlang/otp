...
%% ------------------------------------------------------------
block_internal(Node) ->
    persistent_term:put({?MODULE, Node}, true).

unblock_internal(Node) ->
    case persistent_term:erase({?MODULE, Node}) of
        true ->
...
is_blocked(Node) ->
    persistent_term:get({?MODULE, Node}, false).
...
set_peer_ref(Node, Ref) ->
    persistent_term:put({peer, Node}, Ref).

get_peer_ref(Node) ->
    persistent_term:get({peer, Node}).

get_peer_ref(Node, Default) ->
    persistent_term:get({peer, Node}, Default).
