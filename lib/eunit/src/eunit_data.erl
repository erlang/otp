...
-export_type([tests/0, simple_test/0, testfunction/0,
              ...
              abstract_instantiator/0, proc/0, moduleName/0,
              functionName/0, appName/0, fileName/0, testIterator/0]).
...
-opaque testIterator() :: #iter{}.

-type opts() :: [any()].

-spec iter_init(tests(), [integer()], opts()) -> testIterator().

iter_init(Tests, ParentID, Options) ->
    #iter{tests = Tests, parent = lists:reverse(ParentID), options = Options}.

-spec iter_id(testIterator()) -> [integer()].

iter_id(#iter{pos = N, parent = Ns}) ->
    lists:reverse(Ns, [N]).

-spec iter_next(testIterator()) -> none | {testItem(), testIterator()}.
...
-spec iter_prev(testIterator()) -> none | {testItem(), testIterator()}.
...
-spec next(tests(), opts()) -> none | {testItem(), tests()}.
