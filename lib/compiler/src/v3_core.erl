...
%%  - nomatch_mode is either skip (not matching elements of the generator
%%    have to be silently skipped by the comprehension) or a value X
%%    (to be used in the {badmatch, X} error a non-skipping generator
%%    shall raise).
...
generator(Line, {Generate,Lg,P,E}, Gs, St0) when Generate =:= b_generate;
    ...
    GA = lineno_anno(Lg, St0),
    GAnno = #a{anno=GA},
    %% No-match patterns are very different between skipping and
    %% non-skipping generators. Consider a skipping generator like
    ...
    %%
    %% But for a non-skipping generator there are only three possible
    ...
    try pattern(P, St0) of
        {#ibinary{segments=Segs}=Cp,St1} when Generate =:= b_generate ->
            ...
            Gen = #igen{anno=GAnno,acc_pat=AccPat,acc_guard=Cg,
                        ...
                        arg={Pre,Ce}},
            {Gen,St5};
        {#ibinary{segments=Segs}=Cp,St1} when Generate =:= b_generate_ns ->
            {AccSegs,Tail,TailSeg,St2} = append_tail_segment(Segs, St1),
            AccPat = Cp#ibinary{segments=AccSegs},
            {Cg,St3} = lc_guard_tests(Gs, St2),
...
generator(Line, {Generate,Lg,{map_field_exact,_,K0,V0},E}, Gs, St0) when
    ...
    %%                   %% a skip, even for non-skipping generators
