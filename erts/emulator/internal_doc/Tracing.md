...
Since OTP 27, dynamic trace session that are isolated from each
other can be created. Trace sessions are represented by instances of the struct
`ErtsTraceSession`. The old legacy session (kept for backward compatibility) is
represented by the static instance `erts_trace_session_0`.
