...
trace_on_and_off(Config) when is_list(Config) ->
    ...
    {flags, Flags} = erlang_trace_info(Pid,flags),
    [call,timestamp] = lists:sort(Flags),
    {tracer, Self} = erlang_trace_info(Pid,tracer),
    1 = erlang_trace(Pid, false, [timestamp]),
    {flags,[call]} =  erlang_trace_info(Pid,flags),
    {tracer, Self} = erlang_trace_info(Pid,tracer),
    1 = erlang_trace(Pid, false, [call]),
    {flags,[]} =  erlang_trace_info(Pid,flags),
    {tracer, []} = erlang_trace_info(Pid,tracer),
    unlink(Pid),
    exit(Pid,kill),
    ok.
...
do_trace_bif(Flags) ->
    ...
    erlang_trace_pattern({erlang,'_','_'}, [], Flags),
    Pid ! {do_bif, time, []},
...
do_trace_bif_timestamp(Flags, TsType, TsFlags) ->
    ...
    Pid = spawn_link(?MODULE, bif_process, []),
    1 = erlang_trace(Pid, true, [call]++TsFlags),
    erlang_trace_pattern({erlang,'_','_'}, [], Flags),
    ...
    1 = erlang_trace(Pid, false, TsFlags),
...
do_trace_bif_return(TsType, TsFlags) ->
    io:format("Testing with TsType=~p TsFlags=~p~n", [TsType, TsFlags]),
    Pid = spawn_link(?MODULE, bif_process, []),
    1 = erlang_trace(Pid, true, [call,return_to]++TsFlags),
    erlang_trace_pattern({erlang,'_','_'}, [{'_',[],[{return_trace}]}],
                         [local]),
    ...
    erlang_trace_pattern({erlang,'_','_'}, false, [local]),
    ok.
...
trace_info_old_code(Config) when is_list(Config) ->
    ...
    {traced,undefined} = erlang_trace_info(MFA, traced),
