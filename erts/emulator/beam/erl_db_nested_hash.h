...
typedef struct nested_hash_db_term {
    ...
    int nkitems; /* length of knext/kprevious chain (includes INVALID_HASH terms) */
    LinearHashTable *lht;
...
typedef struct nested_db_term {
    ...
    HashValue ohvalue;          /* stored hash value of the whole dbterm */
    NestedHashDbTerm *hdbterm;
} NestedDbTerm;
...
void db_unfix_table_nhash(DbTableNestedHash *tb /* [in out] */);
Uint db_kept_items_nhash(DbTableNestedHash *tb);

/* Interface for meta pid table */
int db_create_nhash(Process *p,
                    DbTable *tbl /* [in out] */);
...
void db_calc_stats_nhash(DbTableNestedHash* tb, DbNestedHashStats*);
