...
{
    ErtsRecvMarker *res_mark;

#ifdef DEBUG
    ASSERT(mark);
    switch (from) {
    case ERTS_PRIO_Q_MARK_START:
        ASSERT(mark->mark_type == ERTS_RECV_MARKER_TYPE_PRIO_Q_START);
        break;
    case ERTS_PRIO_Q_MARK_END:
        ASSERT(mark->mark_type == ERTS_RECV_MARKER_TYPE_PRIO_Q_END);
        break;
    case ERTS_PRIO_Q_MARK_CONT:
        ASSERT(mark->mark_type == ERTS_RECV_MARKER_TYPE_PRIO_Q_CONT);
        break;
    default:
        ASSERT(0);
        break;
    ...
    res_mark = &mark[to - from];

#ifdef DEBUG
    switch (to) {
    case ERTS_PRIO_Q_MARK_START:
        ASSERT(res_mark->mark_type == ERTS_RECV_MARKER_TYPE_PRIO_Q_START);
        break;
    case ERTS_PRIO_Q_MARK_END:
        ASSERT(res_mark->mark_type == ERTS_RECV_MARKER_TYPE_PRIO_Q_END);
        break;
    case ERTS_PRIO_Q_MARK_CONT:
        ASSERT(res_mark->mark_type == ERTS_RECV_MARKER_TYPE_PRIO_Q_CONT);
        break;
    default:
        ASSERT(0);
        break;
...
static ERTS_INLINE void
msgq_remove_prioq_cont_marker(Process *c_p)
{
    ...
    pq_contp = pqmark2pqmark(pq_start,
...
erts_msgq_remove_leading_recv_markers_set_save_first(Process *c_p)
{
    ...
    if (ERTS_MQ_GET_SAVE_INFO(c_p) == FS_SET_SAVE_INFO_MARK)
        msgq_remove_prioq_cont_marker(c_p);
                                  ...
                                  Eterm message, Eterm token)
{
    ...
    if (c_p->sig_qs.flags & FS_PRIO_MQ_SAVE) {
        ...
        if (set_save) {
            ...
            ErtsRecvMarker *pq_start = (ErtsRecvMarker *) c_p->sig_qs.first;
            ErtsRecvMarker *pq_cont = pqmark2pqmark(pq_start, 
                                                    ERTS_PRIO_Q_MARK_START,
                                                    ERTS_PRIO_Q_MARK_CONT);
             ...
             */
            if (pq_cont->in_msgq) {
                remove_innerq_sig(c_p, (ErtsMessage *) pq_cont, pq_cont->prev_next);
                pq_cont->in_msgq = pq_cont->in_sigq = 0;
            }
...
install_prio_msg_queue(Process *c_p)
{
    ...
    if (c_p->sig_qs.flags & FS_PRIO_MQ_PENDING_RM) {
        /* we just aborted an ongoing removal of the prio queue... */
        ASSERT(c_p->sig_qs.first
               && ERTS_SIG_IS_RECV_MARKER(c_p->sig_qs.first)
               && (((ErtsRecvMarker *) c_p->sig_qs.first)->mark_type
                   == ERTS_RECV_MARKER_TYPE_PRIO_Q_START));
