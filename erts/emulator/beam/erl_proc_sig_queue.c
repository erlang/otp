                ...
                ErtsSignalInQueue* dest_queue)
{
    ...
    *this = first;
    ...
    if (!dest_queue->nmsigs.next) {
        ...
        if (is_to_buffer) {
            /*
             * Read state first to avoid write overhead when it is
             * unecessary.
             *
             * Release barrier is used since we do not want this
             * operation to be reordered with previous write to
             * buffers->nonmsg_slots.
             */
            state = erts_atomic32_read_relb(&rp->state);
                                   ...
                                   erts_aint32_t in_state)
{
    return enqueue_signals(rp, first, last, last_next, num_msgs, in_state,
                           in_state & ERTS_PSFLG_OFF_HEAP_MSGQ, &rp->sig_inq);
}
                                          ...
                                          int is_nonmsg_signal_enqueue)
{
    ...
    if ((receiver_locks & ERTS_PROC_LOCK_MSGQ) ||
        ...
        /*
         * Use the sender process ID to hash to an outer signal queue
         * buffer. This guarantees that all signals from the same
         * process are ordered in send order.
         */
        Uint to_hash =
            (sender == NULL ? 0 : internal_pid_number(sender->common.id));
        ...
        Uint32 state;
        /*
         * Multiple signals or is_nonmsg_signal_enqueue means that one
         * of the signals is a non-message signal
         */
        int is_nonmsg_signal = is_nonmsg_signal_enqueue || !(last == &first->next);
        ...
        if (&buffer->queue.first == buffer->queue.last) {
            ...
            nonempty_slots_before =
                erts_atomic64_read_bor_nob(&buffers->nonempty_slots,
                                           ((Uint64)1) << slot);
        }

        if (is_nonmsg_signal && !buffer->queue.nmsigs.next) {
            /*
             ...
             */
            erts_atomic64_read_bor_nob(&buffers->nonmsg_slots,
                                       ((Uint64)1) << slot);
        }

        if (last == &first->next && !is_nonmsg_signal) {
            /*
...
static Uint erts_proc_sig_queue_flush_buffer(Process* proc,
    ...
    erts_atomic64_read_band_nob(&buffers->nonempty_slots,
                                ~(((Uint64)1) << buffer_index));
    erts_proc_sig_queue_unlock_buffer(buf);
    return no_of_enqueues;
}
...
erts_proc_sig_queue_flush_buffers(Process* proc) {
    ...
    erts_atomic64_set_nob(&buffers->nonmsg_slots, 0);
    if (nonempty_slots > 0) {
        ...
        /*
         * A write barrier is needed here so that the statment
         * erts_atomic64_set_nob(&buffers->nonmsg_slots, 0) above does
         * not get reordered with future writes to the
         * ERTS_PSFLG_SIG_IN_Q flag.
         */
...
void erts_proc_sig_queue_maybe_install_buffers(Process* p, erts_aint32_t state) {
    ...
    erts_atomic64_init_nob(&buffers->nonmsg_slots, (Uint64)0);
    buffers->no_of_enqueues = 0;
    buffers->no_of_rounds = 0;
    /* Initialize  slots */
    for(i = 0; i < ERTS_PROC_SIG_INQ_BUFFERED_NR_OF_BUFFERS; i++) {
        buffers->slots[i].alive = 1;
        erts_mtx_init(&buffers->slots[i].lock,
