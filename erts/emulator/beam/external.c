                         ...
                         Sint reds)
{
    ...
    if ((~dflags & DFLAG_ALTACT_MSG)
        && ep[0] == SMALL_TUPLE_EXT
        && (ep[1] == 4 || ep[1] == 5)
        && ep[2] == SMALL_INTEGER_EXT
        && ep[3] == DOP_ALTACT_MSG_SEND) {
        byte *ptr;
        int dist_op;
        Uint32 flags;
        int tuple_size = ep[1];
        ...
        /*
         * The receiver does not understand alternate action messages...
         *
        ...
        if (flags & ERTS_DOP_ALTACT_MSG_FLG_ALIAS) {
            dist_op = ((~dflags & DFLAG_ALIAS)
                       ? 0 /* drop it... */
                       : ((flags & ERTS_DOP_ALTACT_MSG_FLG_TOKEN)
                          ? DOP_ALIAS_SEND_TT
                          : DOP_ALIAS_SEND));
        }
        else if (flags & ERTS_DOP_ALTACT_MSG_FLG_NAME) {
            dist_op = ((flags & ERTS_DOP_ALTACT_MSG_FLG_TOKEN)
                       ? DOP_REG_SEND_TT
                       : DOP_REG_SEND);
        }
        ...
        case DOP_SEND_SENDER_TT:
            /* {DOP_SEND_SENDER_TT, SenderPid, ToPid, TraceToken} */
        ...
        case DOP_SEND_TT:
            ...
            /*
             * These require a bit more rewriting than alias and
             ...
             */
        {
            ...
            goto altact_msg_fallback_common;
        }

        case 0: {
            int i;
            /* Drop message and send a tick instead... */
            for (i = 1; i < ob->eiov->vsize; i++) {
                if (ob->eiov->binv[i])
                    driver_free_binary(ob->eiov->binv[i]);
            }
