...
{
    if (init->atype == ERTS_ALC_S_AFIT)
	init->atype = ERTS_ALC_S_GOODFIT;
}
...
strategy_support_carrier_migration(struct au_init *auip)
{
     ...
     */
    return auip->atype == ERTS_ALC_S_FIRSTFIT;
}
...
adjust_carrier_migration_support(struct au_init *auip)
{
	...
	if (!strategy_support_carrier_migration(auip)) {
	    /* Default to aoffcbf */
	    auip->atype = ERTS_ALC_S_FIRSTFIT;
		   ...
		   ErtsAllocatorState_t *state)
{
	...
	if (!init->thr_spec)
		...
		if (i != 0)
		    ...
		    if (atype == ERTS_ALC_S_AFIT)
			atype = ERTS_ALC_S_GOODFIT;
		    init->init.util.ts = 1;
		}
	      ...
	      int u_switch)
{
    ...
    case 'a':
		...
		auip->atype = ERTS_ALC_S_GOODFIT;
	    }
	    else if (sys_strcmp("af", alg) == 0) {
		auip->atype = ERTS_ALC_S_AFIT;
	    }
		...
		bad_value(param, sub_param + 1, alg);
            ...
            auip->init.util.alloc_strat = auip->atype;
	    if (!strategy_support_carrier_migration(auip))
...
alcu_size(ErtsAlcType_t alloc_no, ErtsAlcUFixInfo_t *fi, int fisz)
{
    ...
    /* We have to visit every type to gather information on blocks that were
     * allocated on a carrier that's owned by a different allocator type. */
...
UWord erts_alc_test(UWord op, UWord a1, UWord a2, UWord a3)
{
	...
	case 0xf03: {
	    ...
	    init.init.util.alloc_strat = init.atype;
	    init.init.util.ts = 1;
	    if ((char **) a3) {
		char **argv = (char **) a3;
		int i = 0;
