 ...
 *
 * A yielding version of c2int_parse() (the large-input string/list -> bignum
 * parser) is generated by YCF into "big.ycf.h" by the ERTS build system; the
 * generation is defined in "$ERL_TOP/erts/emulator/Makefile.in". See
 * "$ERL_TOP/erts/emulator/internal_doc/AutomaticYieldingOfCCode.md" and
 * "$ERL_TOP/erts/lib_src/yielding_c_fun/README.md" for the limitations YCF
 * imposes on the transformed code.
 *
 * The generated "big.ycf.h" is #included further down, after the file-local
 * helpers and macros (struct dc_pow_cache, I_*, big_norm, MOVE_DIGITS, ...)
 * that the transformed code references have been defined.
 */
 ...
 * loads (lane 0 after a load is the byte at the lowest address) and that one
 * ErtsDigit holds exactly 16 hex or 64 binary digits, i.e. 64-bit digits.
 * This build-time constant gates the pack fast path; on big-endian or 32-bit
 * digits it is 0 and the branch that uses it folds away, falling back to the
 * scalar accumulator.
 */
#ifdef WORDS_BIGENDIAN
...
/*
 * ============================================================================
 * String/list -> bignum parsing (the large-input path)
 * ============================================================================
 *
 * For inputs that don't fit in a small, the value is parsed entirely in C
 * using a divide-and-conquer combine, yielding cooperatively via YCF (Yielding
 * C Fun). This mirrors the algorithm that previously lived in Erlang
 * (erlang:big_binary_to_int/2 and helpers): the digit string is chopped into
 * "limbs" of K = get_digits_per_small(base) base-`base` digits (each limb < the
 * radix base^K and thus fits in one ErtsDigit), and the limbs are then combined
 * pairwise with a radix-squaring tree:
 *
 *     [l0 l1 l2 l3 ...]  (l0 most significant)
 *  -> [l0*R + l1, l2*R + l3, ...]            (radix R   = base^K)
 *  -> [(l0*R+l1)*R^2 + (l2*R+l3), ...]       (radix R^2 = base^2K)
 *  -> ...                                     until a single value remains.
 *
 * The heavy multiplications use the existing Karatsuba routine
 * (I_mul_karatsuba); the combine yields between tree nodes, exactly as the old
 * Erlang version yielded between reduction-counted `combine` steps. A single
 * top-level multiply of the two largest halves is one non-yieldable chunk, the
 * same as it was in Erlang (a single `B*Radix + A`).
 *
 * The actual yielding version of c2int_parse() is generated by YCF into
 * big.ycf.h; see the BIF wrappers and erts/emulator/Makefile.in.
 */

/*
 * Maximum number of base-`base` digits whose value is guaranteed to fit in a
 * bignum (BIG_ARITY_MAX ErtsDigit words). A value with L base-`base` digits is
 * < base^L, which needs ceil(L*log2(base)/D_EXP) words; so the largest L that
 * cannot overflow BIG_ARITY_MAX words is floor(BIG_ARITY_MAX*D_EXP/log2(base)).
 *
 * This is an exact, per-base early-out replacing the two coarse thresholds that
 * previously lived in erlang:big_binary_to_int/2 (4_194_304 for all bases and
 * 1_262_611 for base >= 10, which over-accepted bases 3..9). A string of
 * exactly this many digits can still, at the boundary, round up to one word too
 * many; big_norm()'s BIG_ARITY_MAX check is the exact backstop and turns that
 * into am_system_limit, so this table only needs to be a safe fast path.
 *
 * Wo.Alpha formula (64-bit): Table[Floor[65535*64/Log2[n]], {n, 2, 36}]
 *           (32-bit): Table[Floor[131071*32/Log2[n]], {n, 2, 36}]
 */
static const Uint c2int_max_digits_lookup[36-1] = {
#if (SIZEOF_VOID_P == 4)
    4194272, 2646290, 2097136, 1806374, 1622565, 1494029, 1398090, 1323145,
    1262601, 1212416, 1169962, 1133452, 1101623, 1073557, 1048568, 1026130,
    1005838, 987369, 970463, 954910, 940539, 927205, 914788, 903187, 892314,
    882096, 872469, 863377, 854771, 846609, 838854, 831471, 824432, 817711,
    811282
#elif (SIZEOF_VOID_P == 8)
    4194240, 2646270, 2097120, 1806360, 1622553, 1494018, 1398080, 1323135,
    1262592, 1212407, 1169953, 1133443, 1101615, 1073549, 1048560, 1026123,
    1005831, 987361, 970455, 954903, 940532, 927198, 914781, 903180, 892308,
    882090, 872463, 863371, 854765, 846603, 838848, 831465, 824426, 817704,
    811276
#else
    #error "Please produce a lookup table for the new architecture"
#endif
...
static ERTS_INLINE Uint get_max_digits(Uint base) {
    return c2int_max_digits_lookup[base-2];
}

/*
 * Number of limbs at or below which the small-result Horner fast path is used
 * instead of the radix-power divide-and-conquer. Chosen so the fast path covers
 * common "just over a small" integers without paying the ladder/ping-pong setup.
 */
#define C2INT_HORNER_MAX_LIMBS 16

/*
 * base^n as a single ErtsDigit. Only valid when base^n fits in a word, which
 * holds for n = get_digits_per_small(base) (radix0 = base^K < 2^(SMALL_BITS-1)).
 */
static ERTS_INLINE ErtsDigit compute_base_to_n_word(Uint base, Uint n)
{
    ErtsDigit r = 1;
    while (n--) {
        r *= (ErtsDigit) base;
...
/*
 * If base is a power of two (2,4,8,16,32), return log2(base) (the number of
 * bits each digit contributes); otherwise return 0. Such bases need no
 * multiplication to parse: each digit is a fixed-width bit field, so the value
 * is built by bit-packing in linear time.
 */
static ERTS_INLINE int c2int_bits_per_digit_pow2(Uint base)
{
    switch (base) {
    case 2:  return 1;
    case 4:  return 2;
...
/*
 * Build the radix-power ladder vals[level] = radix0 ^ (2^level), where
 * radix0 = base^K. vals[0] is computed directly; each subsequent level is the
 * square of the previous one. Reuses compute_base_to_n() and I_sqr(), the same
 * approach as build_dc_powers() for the inverse (int->string) direction.
 *
 * Only `vals`/`sizes`/`backing`/`meta_alloc` of the dc_pow_cache are used here;
 * the Barrett reciprocal fields (mus/mu_*) are for division only and are left
 * NULL, so free_dc_powers() cleans up correctly.
 */
static int
c2int_build_radix_powers(struct dc_pow_cache *c, int base, Uint k, int n_levels)
{
    Uint total_size;
    Uint top_width;
    Uint top_size_est;
    ...
    /* Width (in base-`base` digits) of the top level. */
    top_width = k;
    for (i = 1; i < n_levels; i++) {
        if (top_width > (UINT_MAX / 2)) {
            return 0;
        }
        top_width *= 2;
    }

    /* Per-level size upper bound in ErtsDigits, then geometric sum. */
    top_size_est = (Uint) (((double) top_width * lookup_log2(base)) / D_EXP) + 2;
    total_size = 2 * top_size_est + (Uint) n_levels * 2;

    meta_size = (sizeof(Uint) + sizeof(dsize_t) + sizeof(ErtsDigit *)) *
                (Uint) n_levels;
    meta_buf = (char *) erts_alloc(ERTS_ALC_T_C2INT_SCRATCH, meta_size);
    c->meta_alloc = meta_buf;
    c->widths = (Uint *) meta_buf;
    c->sizes  = (dsize_t *) (meta_buf + sizeof(Uint) * (Uint) n_levels);
    c->vals   = (ErtsDigit **) (meta_buf +
                                (sizeof(Uint) + sizeof(dsize_t)) *
                                (Uint) n_levels);
    c->backing = (ErtsDigit *) erts_alloc(ERTS_ALC_T_C2INT_SCRATCH,
                                          sizeof(ErtsDigit) * total_size);

    /* Level 0: radix0 = base^K computed directly. */
    cur = c->backing;
    c->vals[0] = cur;
    c->widths[0] = k;
    c->sizes[0] = compute_base_to_n(base, k, cur);
    cur += c->sizes[0] + 1;
    ASSERT(cur <= c->backing + total_size);

    /*
     * Levels 1..n-1: square the previous level. Use Karatsuba (sub-quadratic)
     * rather than the schoolbook I_sqr: the high levels square ~n/4-word values,
     * so an O(n^2) square there would dominate the whole parse (it was ~70% of
     * runtime before this was switched to Karatsuba). I_mul_karatsuba handles
     * the x==y case for sizes >= 16 and falls back to I_sqr below that.
     */
    for (i = 1; i < n_levels; i++) {
        ErtsDigit *prev = c->vals[i-1];
        dsize_t prev_sz = c->sizes[i-1];
...
/*
 * Free a radix-power ladder built by c2int_build_radix_powers(). Mirrors
 * free_dc_powers() but releases the ERTS_ALC_T_C2INT_SCRATCH allocations used
 * by the (yielding) parser. Only `backing`/`meta_alloc` are ever set here.
 */
static void c2int_free_radix_powers(struct dc_pow_cache *c)
{
    if (c->backing) {
        erts_free(ERTS_ALC_T_C2INT_SCRATCH, c->backing);
    }
    if (c->meta_alloc) {
        erts_free(ERTS_ALC_T_C2INT_SCRATCH, c->meta_alloc);
    }
}

/*
 * Build the parser's final result term on the process heap: a bignum (or
 * small) from the sz accumulated digit words at xp, optionally wrapped in a
 * {Int,Rest} tuple for the list source. Returns am_system_limit if the value
 * cannot fit in a bignum.
 *
 * big_norm() either writes a BIG_NEED_SIZE(sz)-word bignum and returns it, or
 * collapses to an immediate small (consuming no heap). Over-allocate the
 * maximum that might be needed, then HRelease the words big_norm/the tuple did
 * not actually use, so no uninitialized heap words are left live (which would
 * trip the heap check).
 */
static Eterm c2int_make_result(Process *p, ErtsDigit *xp, dsize_t sz,
                               int sign, int want_tuple, Eterm rest_out)
{
    Eterm res;
    Uint need;
    Eterm *hp_start;
    ...
    if (BIG_NEED_SIZE(sz) - 1 > BIG_ARITY_MAX) {
        return am_system_limit;
    }

    need = BIG_NEED_SIZE(sz) + (want_tuple ? 3 : 0);
    hp_start = HAlloc(p, need);
    hp_end = hp_start + need;
    hp = hp_start;

    MOVE_DIGITS(BIG_V(hp), xp, sz);
    res = big_norm(hp, sz, (short) sign);
    if (is_nil(res)) {
        HRelease(p, hp_end, hp_start);
        return am_system_limit;
    }
    /* Advance past the bignum only if one was actually built. */
    hp = is_big(res) ? hp + BIG_NEED_SIZE(sz) : hp_start;
    if (want_tuple) {
        Eterm tpl = TUPLE2(hp, res, rest_out);
        hp += 3;
        res = tpl;
    }
...
/*
 * Yielding parser shared by erts_internal:binary_to_integer/2 and
 * erts_internal:list_to_integer/2 for the large (bignum) path. The source is a
 * binary/bitstring or a cons list, distinguished by the term type of
 * bif_args[0]. bif_args[1] is the base. Returns the parsed integer Eterm, or a
 * sentinel atom (am_badarg / am_no_integer / am_system_limit). For the list
 * source a successful parse returns a {Int,Rest}
 * tuple (the partial-parse contract consumed by string:list_to_integer/1);
 * the binary source returns the bare integer.
 *
 * Phases, all yielding:
 *   1. Strip a leading '+'/'-' sign and trim leading '0' digits (and, for
 *      lists, detect the {0,Rest} / no_integer cases).
 *   2. Count the run of valid digits (lists stop at the first non-digit; for
 *      binaries any invalid byte is badarg). This determines limb layout.
 *   3. Fill the limb buffer most-significant-first, so the short (ragged) limb
 *      is first and every other limb holds exactly K = get_digits_per_small
 *      digits (value < radix = base^K, fits one ErtsDigit).
 *   4. Combine the limbs with the radix-squaring tree (Karatsuba multiplies),
 *      and finalize the bignum on the process heap once.
 *
 * This function is transformed by YCF into a yielding version; see the note at
 * the top of this section. YCF restrictions apply (no pointers to the C stack
 * live across a yield, no macro-hidden control flow). All working storage lives
 * in TMP allocations referenced from YCF-tracked locals; the ON_DESTROY_STATE
 * block frees them if the process is killed mid-parse.
 *
 * Note on the binary byte pointer across yields: the process's GC is disabled
 * while the BIF is trapping (erts_ycf_trap_driver), so the binary cannot move;
 * `temp_alloc` (if erts_get_aligned_binary_bytes copied) is a TMP allocation
 * tracked as a local and freed both on return and in ON_DESTROY_STATE.
 */
static Eterm c2int_parse(Process *p, Eterm *bif_args)
{
    struct dc_pow_cache cache;
    Eterm input = bif_args[0];
    Uint base;
    ...
    Eterm rest_out = THE_NON_VALUE;
    ErtsDigit *limbs = NULL;   /* scanned limbs, most significant first */
    ErtsDigit *acc = NULL;     /* accumulator for the small-result fast path */
    ErtsDigit *pow2_w = NULL;  /* bit-packed words for the power-of-two path */
    ErtsDigit *src = NULL;     /* current level's packed limb arrays */
    Uint *src_off = NULL;      /* src_off[i] = offset of limb i in src */
    dsize_t *src_sz = NULL;    /* src_sz[i] = size (digits) of limb i */
    ErtsDigit *dst = NULL;
    Uint *dst_off = NULL;
    dsize_t *dst_sz = NULL;
    ErtsDigit *prod = NULL;    /* scratch for one multiply */
    Uint n;                    /* number of limbs at the current level */
    Uint n_limbs;              /* number of limbs after scanning */
    Uint cap;                  /* digit capacity of src/dst buffers */
    int level;
    int n_levels;
    Uint k;
    Uint i;
    ...
    Eterm *hp;
    dsize_t final_sz;
    Uint n_digits;             /* count of valid digits to consume */
    Uint first_limb_digits;    /* digits in the ragged most-significant limb */
    Uint max_digits;
    Eterm scan;                /* list cursor during the fill pass */

    base = unsigned_val(bif_args[1]);
    k = (Uint) get_digits_per_small(base);
    is_list_src = is_list(input) || is_nil(input);

    /*
     * Initialize the radix-power cache pointers so the shared cleanup
     * (c2int_free_radix_powers) is safe even on paths that never build it
     * (the small-result fast path and early returns).
     */
    cache.backing = NULL;
    cache.mu_backing = NULL;
    cache.meta_alloc = NULL;

    /*
     * Phase 1+2: sign, leading-zero trim, and valid-digit count. For the list
     * source we also remember where the digit run ends (the rest tail).
     */
    n_digits = 0;
    if (is_list_src) {
        Eterm l = input;
        ...
        if (is_nil(l)) {
            return am_no_integer;
        }
        /* Strip one leading sign char. */
        if (is_list(l)) {
            Eterm car = CAR(list_val(l));
            if (car == make_small('-')) {
                sign = 1;
                l = CDR(list_val(l));
            } else if (car == make_small('+')) {
                l = CDR(list_val(l));
            }
        }
        /* Skip leading '0's. */
        while (is_list(l)) {
            Eterm car = CAR(list_val(l));
            if (is_not_small(car) || unsigned_val(car) != '0') {
                break;
            }
            seen_zero = 1;
            l = CDR(list_val(l));
        ...
        /* Count the run of valid digits. */
        scan = l;
        while (is_list(scan)) {
            Eterm car = CAR(list_val(scan));
            byte c;
            if (is_not_small(car)) {
                break;
            }
            c = (byte) unsigned_val(car);
            if (c2int_is_invalid_char(c, base)) {
                break;
            }
            n_digits++;
            scan = CDR(list_val(scan));
            YCF_CONSUME_REDS(1);
        }
        if (n_digits == 0) {
            /* No non-zero digits. */
            if (seen_zero) {
                /* "0...0" (then EOL or a non-digit): value is small 0. */
                rest_out = l;
                hp = HAlloc(p, 3);
                return TUPLE2(hp, make_small(0), rest_out);
            }
            return am_no_integer;
        }
        ...
        /*
         * erts_get_aligned_binary_bytes() may return an ERTS_ALC_T_TMP copy
        ...
        /*
         * Copy digits to a yield-safe buffer and drop the TMP copy *before* the
         * (yielding) validation loop, so no ERTS_ALC_T_TMP allocation is ever
         * held across a yield.
         */
        bin_copy = (byte *) erts_alloc(ERTS_ALC_T_C2INT_SCRATCH, sz);
        sys_memcpy(bin_copy, b, sz);
        erts_free_aligned_binary_bytes(temp_alloc);
        temp_alloc = NULL;
        bytes = bin_copy;
        size = sz;
    ...
    /*
     * Fast system_limit early-out: exact per-base digit cap. The final
     * big_norm() BIG_ARITY_MAX check below is the exact backstop for the
     * one-word boundary case.
     */
    max_digits = get_max_digits(base);
    if (n_digits > max_digits) {
        if (temp_alloc != NULL) erts_free_aligned_binary_bytes(temp_alloc);
        if (bin_copy != NULL) erts_free(ERTS_ALC_T_C2INT_SCRATCH, bin_copy);
        return am_system_limit;
    }

    /*
     * Power-of-two fast path: no multiplication needed. Each digit contributes
     * a fixed bpd = log2(base) bits, so the value is built by bit-packing in
     * linear time. Digits are read most-significant first; digit at position dp
     * (0 = most significant) occupies result bits [(n_digits-1-dp)*bpd ..].
     */
    if (c2int_bits_per_digit_pow2(base)) {
        int bpd = c2int_bits_per_digit_pow2(base);
        Uint total_bits = (Uint) n_digits * (Uint) bpd;
        dsize_t nwords = (dsize_t) ((total_bits + D_EXP - 1) / D_EXP);
        Uint dp;
        dsize_t wlen;

        pow2_w = (ErtsDigit *) erts_alloc(ERTS_ALC_T_C2INT_SCRATCH,
                                          sizeof(ErtsDigit) * (nwords + 1));
        for (dp = 0; dp <= (Uint) nwords; dp++) {
            pow2_w[dp] = 0;
        }
        ...
        if (is_list_src) {
            ...
            for (dp = 0; dp < (Uint) n_digits; dp++) {
                byte c = (byte) unsigned_val(CAR(list_val(l)));
                ErtsDigit d = (ErtsDigit) c2int_digit_from_base(c);
            ...
            for (dp = (Uint) n_digits; dp > 0; dp--) {
                ...
                /* Batch the reduction bookkeeping to keep the loop tight. */
        ...
        wlen = nwords;
        while (wlen > 1 && pow2_w[wlen - 1] == 0) {
            wlen--;
        }

        res = c2int_make_result(p, pow2_w, wlen, sign, want_tuple, rest_out);
    } else {

    /*
     * Phase 3: fill the limb buffer. The digits are read most-significant
     * first, but stored *least-significant first* (limbs[0] is the LS group).
     * Every full limb holds exactly K digits; the most-significant group is the
     * ragged one (first_limb_digits <= K) and lands at the highest index. This
     * layout is what the radix-doubling combine below requires: the radix at a
     * level always equals base^(width of the lower limb of each merged pair),
     * and the lone most-significant odd limb (whose width may differ) is only
     * ever the *high* operand, so its own width never has to match the radix.
     */
    n_limbs = (n_digits + k - 1) / k;
    first_limb_digits = n_digits - (n_limbs - 1) * k;
    limbs = (ErtsDigit *) erts_alloc(ERTS_ALC_T_C2INT_SCRATCH, sizeof(ErtsDigit) * n_limbs);

    if (is_list_src) {
        Eterm l = list;
        Uint take = first_limb_digits;
        for (i = 0; i < n_limbs; i++) {
            ErtsDigit limb = 0;
            Uint j;
            for (j = 0; j < take; j++) {
                byte c = (byte) unsigned_val(CAR(list_val(l)));
                limb = limb * (ErtsDigit) base
                       + (ErtsDigit) c2int_digit_from_base(c);
                l = CDR(list_val(l));
            }
            limbs[n_limbs - 1 - i] = limb;   /* MS group -> highest index */
            take = k;
            YCF_CONSUME_REDS(first_limb_digits);
        }
    } else {
        const byte *b = bytes;
        Uint take = first_limb_digits;
        for (i = 0; i < n_limbs; i++) {
            ...
            for (j = 0; j < take; j++) {
                limb = limb * (ErtsDigit) base
                       + (ErtsDigit) c2int_digit_from_base(*b++);
            }
            limbs[n_limbs - 1 - i] = limb;   /* MS group -> highest index */
            take = k;
            YCF_CONSUME_REDS(first_limb_digits);
    ...
    /*
     * Fast path for small results (a handful of limbs): a straight Horner
     * accumulation acc = acc*radix0 + limb, most-significant limb first. This
     * avoids the radix-power ladder, the ping-pong buffers, and most of the
     * scratch allocations, which otherwise dominate for values just over the
     * small/bignum boundary (the common 19..~150 digit case). radix0 = base^K
     * always fits a single ErtsDigit (K = get_digits_per_small), so D_mul does
     * the array*digit multiply.
     */
    if (n_limbs <= C2INT_HORNER_MAX_LIMBS) {
        ErtsDigit radix0 = compute_base_to_n_word(base, k);
        dsize_t acc_sz;
        Uint acc_cap;

        /*
         * acc holds the running value, which is < base^n_digits and so needs
         * ceil(n_digits*log2(base)/D_EXP) words; D_mul and D_add each can append
         * a transient carry word, so add slack.
         */
        acc_cap = (Uint) (((double) n_digits * lookup_log2(base)) / D_EXP)
                  + n_limbs + 4;
        acc = (ErtsDigit *) erts_alloc(ERTS_ALC_T_C2INT_SCRATCH,
                                       sizeof(ErtsDigit) * acc_cap);
        /* Most-significant limb first (limbs[] is LS-first, so descend). */
        acc[0] = limbs[n_limbs - 1];
        acc_sz = 1;
        for (i = n_limbs - 1; i > 0; i--) {
            acc_sz = D_mul(acc, acc_sz, radix0, acc);   /* acc *= radix0 */
            acc_sz = D_add(acc, acc_sz, limbs[i - 1], acc);
            YCF_CONSUME_REDS(acc_sz + 1);
        }
        while (acc_sz > 1 && acc[acc_sz - 1] == 0) {
            acc_sz--;
        }
        res = c2int_make_result(p, acc, acc_sz, sign, want_tuple, rest_out);
    } else {

    /*
     * Digit capacity of each ping-pong buffer. The final value needs
     * ceil(total_digits * log2(base) / D_EXP) words, but a buffer holds a whole
     * *level* of separate limbs laid end to end, and each limb's word size can
     * round up (and I_add may add a carry word) independently. The combined
     * slack across a level is bounded by the number of limbs, so add n_limbs
     * (plus a few words) to the value size.
     */
    {
        double bits = (double) n_digits * lookup_log2(base);
        cap = (Uint) (bits / D_EXP) + n_limbs + 4;
    }

    /* Number of radix-doubling levels needed: ceil(log2(n)). */
    n_levels = 0;
    {
        Uint t = 1;
        while (t < n) { t *= 2; n_levels++; }
    }
    /* We need radix powers for levels 0..n_levels-1. */
    if (!c2int_build_radix_powers(&cache, (int) base, k, n_levels)) {
        erts_free(ERTS_ALC_T_C2INT_SCRATCH, limbs);
        if (temp_alloc != NULL) erts_free_aligned_binary_bytes(temp_alloc);
        if (bin_copy != NULL) erts_free(ERTS_ALC_T_C2INT_SCRATCH, bin_copy);
        return am_system_limit;
    }

    /* Two ping-pong buffers of `cap` digits, plus a product scratch. */
    src = (ErtsDigit *) erts_alloc(ERTS_ALC_T_C2INT_SCRATCH, sizeof(ErtsDigit) * cap);
    dst = (ErtsDigit *) erts_alloc(ERTS_ALC_T_C2INT_SCRATCH, sizeof(ErtsDigit) * cap);
    prod = (ErtsDigit *) erts_alloc(ERTS_ALC_T_C2INT_SCRATCH, sizeof(ErtsDigit) * (cap + 2));
    src_off = (Uint *) erts_alloc(ERTS_ALC_T_C2INT_SCRATCH, sizeof(Uint) * (n + 1));
    src_sz  = (dsize_t *) erts_alloc(ERTS_ALC_T_C2INT_SCRATCH, sizeof(dsize_t) * (n + 1));
    dst_off = (Uint *) erts_alloc(ERTS_ALC_T_C2INT_SCRATCH, sizeof(Uint) * (n + 1));
    dst_sz  = (dsize_t *) erts_alloc(ERTS_ALC_T_C2INT_SCRATCH, sizeof(dsize_t) * (n + 1));

    /* Pack the initial limbs into src (one digit each, trimmed). */
    for (i = 0; i < n; i++) {
        src[i] = limbs[i];
        src_off[i] = i;
        src_sz[i] = 1;          /* a zero limb has size 1 with value 0 */
    }
    erts_free(ERTS_ALC_T_C2INT_SCRATCH, limbs);
    limbs = NULL;

    /*
     * Limbs are least-significant first. Each level merges adjacent pairs
     * (lo = src[i], hi = src[i+1]) -> hi * radix + lo, doubling the radix each
     * level. radix = base^(K * 2^level) always equals base^(width of lo), so
     * the shift is correct; the lone most-significant odd limb is carried up
     * unchanged (it is only ever a high operand, so its width need not match).
     * After each merge node, charge reductions so YCF can yield.
     */
    for (level = 0; level < n_levels; level++) {
        ErtsDigit *radix = cache.vals[level];
        dsize_t radix_sz = cache.sizes[level];
        Uint dst_n = 0;
        Uint dst_cur = 0;       /* next free offset in dst */

        for (i = 0; i + 1 < n; i += 2) {
            ErtsDigit *lo = src + src_off[i];
            dsize_t lo_sz = src_sz[i];
            ErtsDigit *hi = src + src_off[i+1];
            dsize_t hi_sz = src_sz[i+1];
            dsize_t prod_sz;
            dsize_t sum_sz;
            ErtsDigit *out = dst + dst_cur;

            /* prod = hi * radix (Karatsuba wants the larger operand first) */
            if (hi_sz >= radix_sz) {
                prod_sz = I_mul_karatsuba(hi, hi_sz, radix, radix_sz, prod);
            } else {
                prod_sz = I_mul_karatsuba(radix, radix_sz, hi, hi_sz, prod);
            }
            while (prod_sz > 1 && prod[prod_sz - 1] == 0) {
                prod_sz--;
            }

            /* out = prod + lo  (I_add wants xl >= yl) */
            if (prod_sz >= lo_sz) {
                sum_sz = I_add(prod, prod_sz, lo, lo_sz, out);
            } else {
                sum_sz = I_add(lo, lo_sz, prod, prod_sz, out);
            }

            dst_off[dst_n] = dst_cur;
            dst_sz[dst_n] = sum_sz;
            dst_n++;
            dst_cur += sum_sz;
            ASSERT(dst_cur <= cap);

            /* Yield point: charge work proportional to the multiply size. */
            YCF_CONSUME_REDS(hi_sz + radix_sz + 1);
        }

        /* Odd one out: carry the last limb up unchanged. */
        if (i < n) {
            ErtsDigit *odd = src + src_off[i];
            dsize_t odd_sz = src_sz[i];
            ErtsDigit *out = dst + dst_cur;
            MOVE_DIGITS(out, odd, odd_sz);
            dst_off[dst_n] = dst_cur;
            dst_sz[dst_n] = odd_sz;
            dst_n++;
            dst_cur += odd_sz;
        ...
        /* Swap src <-> dst for the next level. */
        {
            ErtsDigit *t1 = src; src = dst; dst = t1;
            Uint *t2 = src_off; src_off = dst_off; dst_off = t2;
            dsize_t *t3 = src_sz; src_sz = dst_sz; dst_sz = t3;
        }
        n = dst_n;
    }
    ...
    ASSERT(n == 1);
    final_sz = src_sz[0];
    while (final_sz > 1 && (src + src_off[0])[final_sz - 1] == 0) {
        final_sz--;
    }

    /* Build the result on the process heap exactly once. */
    res = c2int_make_result(p, src + src_off[0], final_sz, sign,
                            want_tuple, rest_out);
    } /* end else (divide-and-conquer path) */
    } /* end else (non-power-of-two path) */

    /*
     * Scratch cleanup, run when the process is killed mid-parse. YCF requires
     * this block to be spelled out inside the transformed function, and a
     * helper cannot be used because the identifiers must be rewritten to the
     * lifted trap-state fields; keep it in sync with the normal-return
     * cleanup below.
     */
    YCF_SPECIAL_CODE_START(ON_DESTROY_STATE);
    {
        if (temp_alloc != NULL) erts_free_aligned_binary_bytes(temp_alloc);
        if (bin_copy != NULL) erts_free(ERTS_ALC_T_C2INT_SCRATCH, bin_copy);
        if (pow2_w != NULL)  erts_free(ERTS_ALC_T_C2INT_SCRATCH, pow2_w);
        if (limbs != NULL)   erts_free(ERTS_ALC_T_C2INT_SCRATCH, limbs);
        if (acc != NULL)     erts_free(ERTS_ALC_T_C2INT_SCRATCH, acc);
        if (prod != NULL)    erts_free(ERTS_ALC_T_C2INT_SCRATCH, prod);
        if (src != NULL)     erts_free(ERTS_ALC_T_C2INT_SCRATCH, src);
        if (dst != NULL)     erts_free(ERTS_ALC_T_C2INT_SCRATCH, dst);
        if (src_off != NULL) erts_free(ERTS_ALC_T_C2INT_SCRATCH, src_off);
        if (src_sz != NULL)  erts_free(ERTS_ALC_T_C2INT_SCRATCH, src_sz);
        if (dst_off != NULL) erts_free(ERTS_ALC_T_C2INT_SCRATCH, dst_off);
        if (dst_sz != NULL)  erts_free(ERTS_ALC_T_C2INT_SCRATCH, dst_sz);
        c2int_free_radix_powers(&cache);
    }
    YCF_SPECIAL_CODE_END();

    /* Normal-return scratch cleanup; keep in sync with ON_DESTROY_STATE. */
    if (temp_alloc != NULL) erts_free_aligned_binary_bytes(temp_alloc);
    if (bin_copy != NULL) erts_free(ERTS_ALC_T_C2INT_SCRATCH, bin_copy);
    if (pow2_w != NULL)  erts_free(ERTS_ALC_T_C2INT_SCRATCH, pow2_w);
    if (limbs != NULL)   erts_free(ERTS_ALC_T_C2INT_SCRATCH, limbs);
...
/*
 * Pull in the YCF-generated yielding version of c2int_parse(). Included here,
 * after all the file-local helpers it references have been defined.
 */
#include "big.ycf.h"
...
/*
 * Number of base-`base` digits processed per reduction in the yielding parser.
 * The scan phase processes one char per YCF_CONSUME_REDS(1), the combine phase
 * charges per multiply size; this scales both to a reasonable reduction budget.
 */
#define C2INT_ITERATIONS_PER_RED 32

/*
 * Drive the yielding c2int_parse() via erts_ycf_trap_driver. `export_ix` is the
 * BIF's own export, used as the re-trap target; on a continuation the first arg
 * is the trap-state magic ref (handled inside the driver).
 */
static ERTS_INLINE BIF_RETTYPE
c2int_trap(Process *p, Eterm *args, int export_ix)
{
    return erts_ycf_trap_driver(p,
                                args,
                                2,
                                C2INT_ITERATIONS_PER_RED,
                                ERTS_ALC_T_C2INT_TRAP,
                                0,
                                export_ix,
                                c2int_parse_ycf_gen_continue,
                                c2int_parse_ycf_gen_destroy,
                                c2int_parse_ycf_gen_yielding);
}
...
BIF_RETTYPE erts_internal_binary_to_integer_2(BIF_ALIST_2)
{
    ...
    /* Continuation of a trapped large parse. */
    if (is_internal_magic_ref(BIF_ARG_1)) {
        return c2int_trap(BIF_P, BIF__ARGS,
                          BIF_erts_internal_binary_to_integer_2);
    }
    ...
    if (res == am_big) {
        /* Large value: parse fully in C, yielding via YCF. */
        return c2int_trap(BIF_P, BIF__ARGS,
                          BIF_erts_internal_binary_to_integer_2);
    }
    BIF_RET(res);
...
BIF_RETTYPE erts_internal_list_to_integer_2(BIF_ALIST_2)
{
    ...
    if (is_internal_magic_ref(BIF_ARG_1)) {
        return c2int_trap(BIF_P, BIF__ARGS,
                          BIF_erts_internal_list_to_integer_2);
    }
