...
stub_read_export_table(LoaderState* stp)
{
    ...
    if (stp->num_exps > stp->num_functions) {
        BeamLoadError2(stp, "%u functions exported; only %u functions defined",
    ...
    for (i = 0; i < stp->num_exps; i++) {
        ...
        GetInt(stp, 4, n);
        if (n > MAX_REG) {
            BeamLoadError2(stp, "export table entry %u: absurdly high arity %u", i, n);
        }
...
static void
stub_beam_load_finalize_code(LoaderState* stp, ErtsCodeInfo* ci)
{
    unsigned int i;
...
erts_make_stub_module(Process* p, Eterm hipe_magic_bin, Eterm Beam, Eterm Info)
{
    ...
    *fp++ = BeamOpCodeAddr(op_int_code_end);
    ...
    fp = code_base;
    for (i = 0; i < n; i++) {
        stub_beam_load_finalize_code(stp, (ErtsCodeInfo*)fp);
    ...
    if (patch_funentries(Patchlist)) {
        ...
        hipe_beam_load_prepared_free(hipe_stp);
