...
static void subtract_ctx_bin_dtor(Binary *context_bin) {
...
static Eterm subtract_create_trap_state(Process *p,
    ...
    return erts_mk_magic_binary_term(&hp, &MSO(p), state_bin);
}
...
static int subtract_continue(Process *p, ErtsSubtractContext *context) {
    switch (context->stage) {
        ...
        default:
            erts_exit(ERTS_ABORT_EXIT, "unreachable");
    }
}
...
static Eterm subtract(Export *bif_entry, BIF_ALIST_2) {
    ...
    } else if (is_binary(lhs)) {
        ErtsSubtractContext *context;
        void (*dtor)(Binary*);
        Binary *magic_bin;
        Eterm *bin = (Eterm*)binary_val(BIF_ARG_1);

        int res;

        if (*bin != HEADER_PROC_BIN) {
            BIF_ERROR(BIF_P, BADARG);
        }

        magic_bin = ((ProcBin *) binary_val(BIF_ARG_1))->val;
...
BIF_RETTYPE ebif_minusminus_2(BIF_ALIST_2) {
    return subtract(bif_export[BIF_ebif_minusminus_2], BIF_P, BIF__ARGS);
}

BIF_RETTYPE subtract_2(BIF_ALIST_2) {
    return subtract(bif_export[BIF_subtract_2], BIF_P, BIF__ARGS);
}
