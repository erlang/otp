...
*/
static ERTS_INLINE Uint hash_to_ix(DbTableNestedHash* tb, LinearHashTable *lht, HashValue hval)
{
...
*/
static ERTS_INLINE void add_fixed_deletion(DbTableNestedHash* tb, int ix)
{
    erts_aint_t was_next;
    erts_aint_t exp_next;
    NestedFixedDeletion* fixd = (NestedFixedDeletion*) erts_db_alloc(ERTS_ALC_T_DB_FIX_DEL,
							 (DbTable *) tb,
...
static ERTS_INLINE erts_smp_rwmtx_t* RLOCK_HASH(DbTableNestedHash* tb, HashValue hval)
{
...
static ERTS_INLINE erts_smp_rwmtx_t* WLOCK_HASH(DbTableNestedHash* tb, HashValue hval)
{
    if (tb->common.is_thread_safe) {
	return NULL;
    } else {
...
*/
static ERTS_INLINE Sint next_slot(DbTableNestedHash* tb, Uint ix,
				  erts_smp_rwmtx_t** lck_ptr)
{
#ifdef ERTS_SMP
    ix += DB_NESTED_HASH_LOCK_CNT;
    if (ix < NACTIVE(&tb->linearht)) return ix;
...
static ERTS_INLINE Sint next_slot_w(DbTableNestedHash* tb, Uint ix,
				    erts_smp_rwmtx_t** lck_ptr)
{
#ifdef ERTS_SMP
    ix += DB_NESTED_HASH_LOCK_CNT;
...
static ERTS_INLINE void free_term(DbTableNestedHash *tb, NestedHashDbTerm* p)
{
    db_free_term((DbTable*)tb, p, offsetof(NestedHashDbTerm, dbterm));
}
...
static ERTS_INLINE void SET_SEGTAB(DbTableNestedHash* tb, LinearHashTable *lht,
...
static NestedHashDbTerm* next(DbTableNestedHash *tb, Uint *iptr, erts_smp_rwmtx_t** lck_ptr,
                              NestedHashDbTerm **listp, NestedHashDbTerm *klist);
 ...
 */
static int db_first_nhash(Process *p,
                          DbTable *tbl,
                          Eterm *ret);

static int db_next_nhash(Process *p,
			DbTable *tbl,
			Eterm key,
			Eterm *ret);
...
static int db_slot_nhash(Process *p, DbTable *tbl,
...
static int db_select_count_nhash(Process *p, DbTable *tbl,
				Eterm pattern, Eterm *ret);
static int db_select_delete_nhash(Process *p, DbTable *tbl,
				 Eterm pattern, Eterm *ret);

static int db_select_continue_nhash(Process *p, DbTable *tbl,
				   Eterm continuation, Eterm *ret);

static int db_select_count_continue_nhash(Process *p, DbTable *tbl,
					 Eterm continuation, Eterm *ret);

static int db_select_delete_continue_nhash(Process *p, DbTable *tbl,
					  Eterm continuation, Eterm *ret);
static void db_print_nhash(int to,
			  void *to_arg,
			  int show,
			  DbTable *tbl);
...
static void db_foreach_offheap_nhash(DbTable *,
				    void (*)(ErlOffHeap *, void *),
				    void *);

static int db_delete_all_objects_nhash(Process* p, DbTable* tbl);
#ifdef HARDDEBUG
static void db_check_table_nhash(DbTable *tbl);
#endif
...
static ERTS_INLINE void try_shrink(DbTableNestedHash* tb)
{
...
*/
static ERTS_INLINE int has_key(DbTableNestedHash* tb, NestedHashDbTerm* b,
...
static ERTS_INLINE NestedHashDbTerm* new_dbterm(DbTableNestedHash* tb, Eterm obj)
{
    NestedHashDbTerm* p;
    if (tb->common.compress) {
	p = db_store_term_comp(&tb->common, NULL, offsetof(NestedHashDbTerm,dbterm), obj);
    }
    else {
	p = db_store_term(&tb->common, NULL, offsetof(NestedHashDbTerm,dbterm), obj);
    }
    return p;
}

static ERTS_INLINE NestedHashDbTerm* replace_dbterm(DbTableNestedHash* tb, NestedHashDbTerm* old,
					      Eterm obj)
{
    NestedHashDbTerm* ret;
    ASSERT(old != NULL);
    if (tb->common.compress) {
	ret = db_store_term_comp(&tb->common, &(old->dbterm), offsetof(NestedHashDbTerm,dbterm), obj);
    }
    else {
	ret = db_store_term(&tb->common, &(old->dbterm), offsetof(NestedHashDbTerm,dbterm), obj);
    }
    return ret;
}

/*
 * It is assumed that the NestedHashDbTerm looked-up for is present in
 * the nested LinearHashTable.
 */
static NestedDbTerm **
get_nested_dbterm(DbTableNestedHash *tb, LinearHashTable *lht, NestedHashDbTerm *hdb)
{
    int nix;
    HashValue ohval;
    NestedDbTerm *ntp, **ntpp;
    /* ??? Should ohval be stored in *hdb ??? */
    /* ??? Is this correct ??? */
    ohval = MAKE_HASH(make_tuple_rel(hdb->dbterm.tpl, hdb->dbterm.tpl));
...
put_nested_dbterm(DbTableNestedHash *tb, LinearHashTable *lht, NestedHashDbTerm *hdb)
{
    int nix;
    HashValue ohval;
    NestedDbTerm *ntp, **ntpp;
...
/*
 * It is assumed that the NestedHashDbTerm to remove is present in
 * the nested LinearHashTable.
 */
static ERTS_INLINE void
remove_nested_dbterm(DbTableNestedHash *tb, LinearHashTable *lht, NestedHashDbTerm *hdb)
{
    NestedDbTerm *ntp, **ntpp;
    ntpp = get_nested_dbterm(tb, lht, hdb);
    ntp = *ntpp;
    *ntpp = ntp->next;
 ...
 * the next NestedHashDbTerm.
 * NOTE: It is assumed that bppp, *bppp, **bppp, kpp and *kpp are not NULL
 * Return: 0 -> the new **kpp NestedHashDbTerm has the same key as the removed one's
 *         1 -> the new **kpp NestedHashDbTerm has a different key from the removed one's
 *         2 -> pseudo deletion -- implies case 1 above
 */
/* ??? ERTS_INLINE ??? */
static int
remove_dbterm(DbTableNestedHash *tb, NestedHashDbTerm ***bppp, NestedHashDbTerm **kpp, int pseudo_delete)
{
    NestedHashDbTerm **bpp = *bppp;
    NestedHashDbTerm *bp = *bpp, *kp = *kpp;
    if (bp == kp) {
        if (bp->knext == NULL) {
            ASSERT(bp->nkitems == 1);
            if (pseudo_delete) {
                ...
                *bpp = *kpp = bp->next;
                if (bp->lht != NULL) {
                    remove_nested_dbterm(tb, bp->lht, bp);
            ...
            *bpp = *kpp = bp = bp->knext;
            bp->next = kp->next;
            ASSERT(bp->kprevious == kp);
            bp->kprevious = NULL;
            bp->nkitems = kp->nkitems - 1;
            bp->lht = kp->lht;
            if (bp->lht != NULL) {
                remove_nested_dbterm(tb, bp->lht, kp);
        ...
        ASSERT(kp->kprevious != NULL);
        kp->kprevious->knext = kp->knext;
        --bp->nkitems;
        if (bp->lht != NULL) {
            remove_nested_dbterm(tb, bp->lht, kp);
            try_nested_shrink(tb, bp->lht, bp->nkitems);
        ...
        if (kp->knext == NULL) {
            ...
            *kpp = bp->next;
            ...
            kp->knext->kprevious = kp->kprevious;
            *kpp = kp->knext;
            free_term(tb, kp);
            return 0;
        }
 ...
 * NestedHashDbTerm in the bucket.
...
/*
 * 'bp' and 'kp' are updated to point to the next NestedHashDbTerm
 * in the chain.
 * NOTE: It is assumed that both b and k are not NULL
 */
...
DbTableMethod db_nested_hash =
{
    ...
    db_select_continue_nhash, /* hmm continue_nhash? */
...
static void restore_fixdel(DbTableNestedHash* tb, NestedFixedDeletion* fixdel)
{
    /*int tries = 0;*/
    DEBUG_WAIT();
    ...
    /*erts_fprintf(stderr,"erl_db_nested_hash: restore_fixdel tries=%d\r\n", tries);*/
}
/*
...
restart:
    fixdel = (NestedFixedDeletion*) erts_smp_atomic_xchg_acqb(&tb->fixdel,
							(erts_aint_t) NULL);
    while (fixdel != NULL) {
...
static void create_linear_hash(DbTableNestedHash *tb, LinearHashTable *lht)
{
...
int db_create_nhash(Process *p, DbTable *tbl)
{
	...
	if (tb->common.type & DB_FREQ_READ)
        ...
        /* ??? Shouldn't check for tb->locks == NULL ??? */
        /* ??? or use erts_db_alloc() instead ??? */
	tb->locks =
            (DbTableNestedHashFineLocks*) erts_db_alloc_fnf(ERTS_ALC_T_DB_SEG, /* Other type maybe? */
                                                            (DbTable *) tb,
	...
	for (i=0; i<DB_NESTED_HASH_LOCK_CNT; ++i) {
	    erts_smp_rwmtx_init_opt_x(&tb->locks->lck_vec[i].lck, &rwmtx_opt,
				      "db_nested_hash_slot", make_small(i));
	}
        /* This important property is needed to guarantee that the buckets
         * involved in a grow/shrink operation it protected by the same lock:
	 */
	ASSERT(erts_smp_atomic_read_nob(&tb->linearht->nactive) % DB_NESTED_HASH_LOCK_CNT == 0);
    }
    else { /* coarse locking */
	tb->locks = NULL;
    }
...
static int db_first_nhash(Process *p, DbTable *tbl, Eterm *ret)
{
    DbTableNestedHash *tb = &tbl->nested;
    Uint ix = 0;
    erts_smp_rwmtx_t* lck = RLOCK_HASH(tb,ix);
    NestedHashDbTerm *list;

    for (;;) {
	list = BUCKET(tb,ix);
	if (list != NULL) {
	    if (list->hvalue == INVALID_HASH) {
		list = next(tb, &ix, &lck, &list, list);
	    }
	    break;
	}
...
static int db_next_nhash(Process *p, DbTable *tbl, Eterm key, Eterm *ret)
{
    DbTableNestedHash *tb = &tbl->nested;
    HashValue hval;
    Uint ix;
...
int db_put_nhash(DbTable *tbl, Eterm obj, int key_clash_fail)
{
    ...
    */
                ...
                if (db_eq(&tb->common, obj, &q->dbterm)) {
                    if (q->hvalue == INVALID_HASH) {
                        ...
                        if (q != b) { /* must move to preserve key insertion order */
                            if (q->knext != NULL) {
                                q->knext->kprevious = q->kprevious;
                            }
                            ASSERT(q->kprevious != NULL);
                            q->kprevious->knext = q->knext;
                            q->knext = b;
                            b->kprevious = q;
                            q->kprevious = NULL;
                            q->next = b->next;
    ...
    if (b->hvalue == INVALID_HASH) {
        /*
         * Recycle the root NestedHashDbTerm if pseudo-deleted -- we must
         * do that, as long as only the root NestedHashDbTerm can be
         * pseudo-deleted (i.e., we cannot prepend a new NestedHashDbTerm
         * pushing the pseudo-deleted one deeper in the 'knext' chain)
         */
#ifdef DEBUG
        LinearHashTable *lht=b->lht;
#endif
        erts_smp_atomic_inc_nob(&tb->common.nitems);
        if (b->lht != NULL) {
            remove_nested_dbterm(tb, b->lht, b);
        }
	...
	*bp = q;
        ASSERT(q->lht == lht);
        if (q->lht != NULL) {
            put_nested_dbterm(tb, q->lht, q);
        }
	goto Ldone;
    }
    q = new_dbterm(tb, obj);
    q->next = b->next;
    q->nkitems = b->nkitems + 1;
    q->lht = b->lht;
...
Lnew:
    ...
    *bp = q;
    if (q->lht != NULL) {
        put_nested_dbterm(tb, q->lht, q);
	if (q->nkitems > NACTIVE(q->lht) * (CHAIN_LEN+1)) {
	    nested_grow(tb, q->lht);
	}
    } else if ((tb->common.status & DB_BAG)
               && (q->nkitems > DB_BAG_LHT_LEN)) {
        /*
         * NOTE: Once created, the nested table is removed only
         * when all the elements of the 'knext' chain are deleted.
         */
        q->lht = (LinearHashTable *)
            erts_db_alloc_fnf(ERTS_ALC_T_DB_SEG, /* ??? Use another type ??? */
                              (DbTable *)tb,
                              sizeof(LinearHashTable));
        /* If q->lht == NULL try again next time */
        if (q->lht != NULL) {
            create_linear_hash(tb, q->lht);
            nitems = 0;
            b = q;
            do {
                put_nested_dbterm(tb, q->lht, b);
                ++nitems;
                if (nitems > NACTIVE(q->lht) * (CHAIN_LEN+1)) {
                    nested_grow(tb, q->lht);
                }
                b = b->knext;
            } while (b != NULL);
...
#define RET_TO_BIF(Term, State) do { *ret = (Term); return State; } while(0);
	...
	if ((kcurrent = next(tb, (Uint*)&slot_ix, &lck, &current, kcurrent)) == NULL) {
	    slot_ix = -1; /* EOT */
	    break;
	}
...
/* N.B.: The name 'next' is misleading. It skips HashDbTerms until */
/*       one with a valid key is found -- it will skip none if */
/*       *klist has a valid key. */
static NestedHashDbTerm* next(DbTableNestedHash *tb, Uint *iptr, erts_smp_rwmtx_t** lck_ptr,
                              NestedHashDbTerm **listp, NestedHashDbTerm *klist)
{
    int i = *iptr;
    ...
    for (;;) {
        while (klist != NULL) {
            if (klist->hvalue != INVALID_HASH)
                return klist;
            NEXT_DBTERM(*listp, klist);
        }
        *iptr = i = next_slot(tb, i, lck_ptr);
        if (!i)
            break;
        klist = *listp = BUCKET(tb,i);
    }
    return NULL;
}
...
static void db_finalize_dbterm_nhash(DbUpdateHandle* handle)
{
    ...
    NestedHashDbTerm* oldp = (NestedHashDbTerm*) *(handle->bp);
...
void db_check_table_nhash(DbTable *tbl)
{
    DbTableNestedHash *tb = &tbl->nested;
    NestedHashDbTerm *list, *klist;
    int j, nk;
