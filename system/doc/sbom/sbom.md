...
security framework that describes how software was built and provides guarantees
about the integrity of the build process. A SLSA provenance document answers the
question: *who built this artifact, from what source, using what process?*

Erlang/OTP produces SLSA Level 3 provenance starting with OTP 29. At this level,
the provenance is:

- **Unforgeable** — generated by the SLSA GitHub Generator, a reusable workflow
  that runs in an isolated job and is not accessible to the repository's own
  workflow code.
- **Signed** — the provenance is signed using [Sigstore](https://www.sigstore.dev/),
  which provides a verifiable link between the artifact, the source commit, and
  the tag used to trigger the release.
- **Verifiable** — anyone can independently verify the provenance without
  trusting Ericsson or GitHub, using only the public Sigstore transparency log
  (Rekor).

In each release, Erlang/OTP publishes a provenance file named
`multiple.intoto.jsonl` alongside the release tarballs. This file covers all
release artifacts: the source tarball, the documentation tarballs, and the SBOM
files.

## Verification Of SLSA Provenance

Below we show how to verify the provenance for Erlang/OTP version `29.1` (e.g.,) using
the [`slsa-verifier`](https://github.com/slsa-framework/slsa-verifier) tool and
the GitHub CLI [`gh`](https://github.com/cli/cli).

### `slsa-verifier`

1. Install `slsa-verifier` by following the
   [installation instructions](https://github.com/slsa-framework/slsa-verifier#installation).
2. Download the artifact you want to verify, for example `otp_src_29.1.tar.gz`
   ([here](https://github.com/erlang/otp/releases/download/OTP-29.1/otp_src_29.1.tar.gz)).
3. Download the provenance file `multiple.intoto.jsonl`
   ([here](https://github.com/erlang/otp/releases/download/OTP-29.1/multiple.intoto.jsonl)).
4. Run `slsa-verifier` with the following parameters:

   ```bash
   slsa-verifier verify-artifact \
      --provenance-path "multiple.intoto.jsonl" \
      --source-uri "github.com/erlang/otp" \
      --source-tag "OTP-29.1" \
      "otp_src_29.1.tar.gz"
   ```


   The same provenance file covers all release artifacts. To verify another artifact, for example the SBOM, pass it instead:
...
1. Download the artifact you want to verify, for example `otp_src_29.1.tar.gz`
([here](https://github.com/erlang/otp/releases/download/OTP-29.1/otp_src_29.1.tar.gz)).
2. Run `gh` with the following parameters:
